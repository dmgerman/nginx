...
control|)
block|{
comment|/* create a socket for each worker process */
name|ls
operator|=
name|ngx_array_push
argument_list|(
...
name|ngx_socket_errno
argument_list|,
literal|"getsockopt(SO_TYPE) %V failed"
argument_list|,
operator|&
name|ls
index|[
...
name|ngx_socket_errno
argument_list|,
literal|"getsockopt(SO_REUSEPORT_LB) %V failed, ignored"
argument_list|,
operator|&
name|ls
index|[
...
name|ngx_socket_errno
argument_list|,
literal|"getsockopt(SO_REUSEPORT) %V failed, ignored"
argument_list|,
operator|&
name|ls
index|[
...
name|err
argument_list|,
literal|"getsockopt(TCP_FASTOPEN) %V failed, ignored"
argument_list|,
operator|&
name|ls
index|[
...
condition|)
block|{
comment|/*                  * to allow transition from a socket without SO_REUSEPORT                  * to multiple sockets with SO_REUSEPORT, we have to set                  * SO_REUSEPORT on the old socket before opening new ones                  */
name|int
name|reuseport
init|=
literal|1
...
name|ngx_socket_errno
argument_list|,
literal|"setsockopt(SO_REUSEPORT_LB) %V failed, "
literal|"ignored"
argument_list|,
operator|&
name|ls
...
name|ngx_socket_errno
expr_stmt|;
comment|/*                  * on OpenVZ after suspend/resume EADDRINUSE                  * may be returned by listen() instead of bind(), see                  * https://bugzilla.openvz.org/show_bug.cgi?id=2470                  */
if|if
condition|(
name|err
operator|!=
...
name|ngx_socket_errno
argument_list|,
literal|"setsockopt(TCP_FASTOPEN, %d) %V failed, ignored"
argument_list|,
name|ls
index|[
name|i
...
name|ngx_socket_errno
argument_list|,
literal|"setsockopt(IP_RECVDSTADDR) "
literal|"for %V failed, ignored"
argument_list|,
operator|&
name|ls
...
condition|)
block|{
comment|/*          * if no connections were freed, try to reuse the last          * connection again: this should free it as long as          * previous reuse moved it to lingering close          */
name|ngx_log_debug0
argument_list|(
name|NGX_LOG_DEBUG_CORE
argument_list|,
...
literal|0
argument_list|,
literal|"reusing connection again"
argument_list|)
expr_stmt|;
name|c
operator|->
...
begin_function
name|void
DECL|function|ngx_close_idle_connections (ngx_cycle_t * cycle)
...
begin_function
name|ngx_int_t
DECL|function|ngx_tcp_nodelay (ngx_connection_t * c)
