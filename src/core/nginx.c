...
argument_list|,
literal|"OPEN: %d"
argument|_ ngx_cycle.log->file->fd
argument_list|)
empty_stmt|;
break|break;
block|}
...
begin_comment
unit|int main(int argc, char *const *argv) {     int          i;     ngx_str_t    conf_file;     ngx_log_t   *log;     ngx_pool_t  *pool, *old_pool;     ngx_conf_t   conf;      ngx_max_sockets = -1;      log = ngx_log_init_errlog();      if (ngx_os_init(log) == NGX_ERROR) {         return 1;     }      ngx_max_module = 0;     for (i = 0; ngx_modules[i]; i++) {         ngx_modules[i]->index = ngx_max_module++;     }
comment|/* life cycle */
end_comment

begin_if
unit|{         old_pool = pool;          pool = ngx_create_pool(16 * 1024, log);          ngx_init_array(ngx_listening_sockets,                        pool, 10, sizeof(ngx_listening_t),                        1);          ngx_memzero(&conf, sizeof(ngx_conf_t));          ngx_test_null(conf.args,                       ngx_create_array(pool, 10, sizeof(ngx_str_t)),                       1);          ngx_test_null(ngx_conf_ctx,                       ngx_pcalloc(pool, ngx_max_module * sizeof(void *)),                       1);          conf.ctx = ngx_conf_ctx;         conf.pool = pool;         conf.log = log;         conf.module_type = NGX_CORE_MODULE;         conf.cmd_type = NGX_MAIN_CONF;          conf_file.len = sizeof(NGINX_CONF) - 1;         conf_file.data = NGINX_CONF;          if (ngx_conf_parse(&conf,&conf_file) != NGX_CONF_OK) {             return 1;         }
if|#
directive|if
literal|0
end_if
...
begin_comment
unit|log = (ngx_log_t *) ngx_get_conf(ngx_errlog_module);
comment|/* STUB */
end_comment

begin_endif
unit|log->log_level = NGX_LOG_DEBUG;
endif|#
directive|endif
end_endif

begin_comment
unit|ngx_init_temp_number();          ngx_io = ngx_os_io;          for (i = 0; ngx_modules[i]; i++) {             if (ngx_modules[i]->init_module) {                 if (ngx_modules[i]->init_module(pool) == NGX_ERROR) {                     return 1;                 }             }         }          if (ngx_open_listening_sockets(log) == NGX_ERROR) {             return 1;         }
...
begin_comment
unit|ngx_pre_thread(&ngx_listening_sockets, pool, log);
