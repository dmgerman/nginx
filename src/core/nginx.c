...
begin_typedef
DECL|struct|__anon2a9a379d0108
typedef|typedef
struct|struct
block|{
DECL|member|user
...
directive|if
literal|0
block_content|if (ngx_delete_file(pidfile.name.data) == NGX_FILE_ERROR) {                         ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno,                                       ngx_delete_file_n " \"%s\" failed",                                       pidfile.name.data);                     }
endif|#
directive|endif
endif|#
directive|endif
...
directive|if
literal|0
block_content|if (ngx_change_binary) {                     ngx_change_binary = 0;                     ngx_log_error(NGX_LOG_INFO, cycle->log, 0,                                   "changing binary");                     ngx_exec_new_binary(cycle, argv);
