...
begin_comment
comment|/*  * locatime() and localtime_r() are not Async-Signal-Safe functions, therefore,  * if ngx_time_update() is called by signal handler, it uses the cached  * GMT offset value. Fortunately the value is changed only two times a year.  */
end_comment
...
begin_function
name|void
DECL|function|ngx_time_update (ngx_uint_t use_cached_gmtoff)
name|ngx_time_update
parameter_list|(
name|ngx_uint_t
