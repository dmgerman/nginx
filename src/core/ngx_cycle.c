...
directive|if
literal|0
block_content|cycle->shm.size =
comment|/* STUB */
block_content|ngx_pagesize;     cycle->shm.log = log;      if (ngx_shm_alloc(&cycle->shm) != NGX_OK) {         ngx_destroy_pool(conf.temp_pool);         ngx_destroy_pool(pool);         return NULL;     }      cycle->shm_last = cycle->shm.addr;     cycle->shm_end = cycle->shm.addr + cycle->shm.size;
endif|#
directive|endif
name|conf
operator|.
