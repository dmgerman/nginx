...
begin_comment
comment|/*  * The threads implementation uses the rfork(RFPROC|RFTHREAD|RFMEM) syscall  * to create threads.  All threads use the stacks of the same size mmap()ed  * below the main stack.  Thus the current thread id is determinated through  * the stack pointer.  *  * The mutex implementation uses the ngx_atomic_cmp_set() operation  * to acquire a mutex and the SysV semaphore to wait on a mutex or to wake up  * the waiting threads.  The light mutex does not use semaphore, so after  * spinning in the lock the thread calls sched_yield().  However the light  * mutecies are intended to be used with the "trylock" operation only.  *  * The condition variable implementation uses the SysV semaphore set of two  * semaphores. The first is used by the CV mutex, and the second is used  * by the CV to signal.  *  * This threads implementation currently works on i386 (486+) and amd64  * platforms only.  */
end_comment
...
begin_function
DECL|function|ngx_cv_init (ngx_log_t * log)
name|ngx_cv_t
modifier|*
name|ngx_cv_init
parameter_list|(
...
begin_function
DECL|function|ngx_cv_done (ngx_cv_t * cv)
name|void
name|ngx_cv_done
parameter_list|(
name|ngx_cv_t
...
begin_function
DECL|function|ngx_cv_wait (ngx_cv_t * cv)
name|ngx_int_t
name|ngx_cv_wait
parameter_list|(
name|ngx_cv_t
...
name|ngx_errno
argument_list|,
literal|"semop() failed while waiting on cv "
name|PTR_FMT
argument_list|,
name|cv
argument_list|)
...
begin_function
DECL|function|ngx_cv_signal (ngx_cv_t * cv)
name|ngx_int_t
name|ngx_cv_signal
parameter_list|(
name|ngx_cv_t
...
name|ngx_errno
argument_list|,
literal|"semop() failed while signaling cv "
name|PTR_FMT
argument_list|,
name|cv
argument_list|)
