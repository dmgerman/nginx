...
condition|)
block|{
comment|/*              * sendfilev() is documented to return -1 with errno              * set to EINVAL if svf_len is greater than the file size,              * but at least Solaris 11 returns 0 instead              */
if|if
condition|(
name|file
condition|)
...
literal|0
argument_list|,
literal|"sendfilev() reported that \"%s\" was truncated at %O"
argument_list|,
name|file
operator|->
name|file
...
literal|0
argument_list|,
literal|"sendfilev() returned 0 with memory buffers"
argument_list|)
expr_stmt|;
block|}
return|return
