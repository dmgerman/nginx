...
name|ngx_errno
argument_list|,
literal|"sigprocmask() failed while signaling processes"
argument_list|)
expr_stmt|;
return|return;
block|}
...
name|ngx_errno
argument_list|,
literal|"sigprocmask() failed while respawning processes"
argument_list|)
expr_stmt|;
return|return;
block|}
comment|/*      * to avoid a race condition we can check and set value of ngx_respawn      * only in signal handler or while SIGCHLD is blocked      */
if|if
condition|(
name|ngx_respawn
condition|)
