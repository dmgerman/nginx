...
directive|if
literal|0
block_content|clcfp = (ngx_http_core_loc_conf_t **) cscfp[s]->locations.elts;                  for (l = 0; l< cscfp[s]->locations.nelts; l++) {                     rv = module->merge_loc_conf(cf,                                                 cscfp[s]->ctx->loc_conf[mi],                                                 clcfp[l]->loc_conf[mi]);                     if (rv != NGX_CONF_OK) {                         *cf = pcf;                         return rv;                     }                 }
endif|#
directive|endif
block|}
block|}
...
name|NGX_OK
expr_stmt|;
comment|/* the special find config phase for single handler */
name|ngx_init_array
argument_list|(
name|cmcf
operator|->
