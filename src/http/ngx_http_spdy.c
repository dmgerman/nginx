...
begin_typedef
DECL|struct|__anon2bbebdc60108
typedef|typedef
struct|struct
block|{
DECL|member|hash
...
begin_endif
unit|static u_char *ngx_http_spdy_state_settings(ngx_http_spdy_connection_t *sc,     u_char *pos, u_char *end);
endif|#
directive|endif
end_endif
...
begin_comment
unit|static u_char * ngx_http_spdy_state_settings(ngx_http_spdy_connection_t *sc, u_char *pos,     u_char *end) {     if (sc->entries == 0) {          if (end - pos< NGX_SPDY_SETTINGS_NUM_SIZE) {             return ngx_http_spdy_state_save(sc, pos, end,                                             ngx_http_spdy_state_settings);         }          sc->entries = ngx_spdy_frame_parse_uint32(pos);          pos += NGX_SPDY_SETTINGS_NUM_SIZE;         sc->length -= NGX_SPDY_SETTINGS_NUM_SIZE;          if (sc->length< sc->entries * NGX_SPDY_SETTINGS_PAIR_SIZE) {
comment|/* TODO logging */
end_comment

begin_endif
unit|return ngx_http_spdy_state_protocol_error(sc);         }          ngx_log_debug1(NGX_LOG_DEBUG_HTTP, sc->connection->log, 0,                        "spdy SETTINGS frame consists of %ui entries",                        sc->entries);     }      while (sc->entries) {         if (end - pos< NGX_SPDY_SETTINGS_PAIR_SIZE) {             return ngx_http_spdy_state_save(sc, pos, end,                                             ngx_http_spdy_state_settings);         }          sc->entries--;          pos += NGX_SPDY_SETTINGS_PAIR_SIZE;         sc->length -= NGX_SPDY_SETTINGS_PAIR_SIZE;     }      return ngx_http_spdy_state_complete(sc, pos, end); }
endif|#
directive|endif
end_endif
...
name|u_char
modifier|*
DECL|function|ngx_http_spdy_state_noop (ngx_http_spdy_connection_t * sc,u_char * pos,u_char * end)
name|ngx_http_spdy_state_noop
parameter_list|(
name|ngx_http_spdy_connection_t
modifier|*
...
DECL|struct|__anon2bbebdc60308
specifier|static
specifier|const
struct|struct
block|{
...
specifier|static
name|ngx_int_t
DECL|function|ngx_http_spdy_parse_url (ngx_http_request_t * r)
name|ngx_http_spdy_parse_url
parameter_list|(
name|ngx_http_request_t
modifier|*
