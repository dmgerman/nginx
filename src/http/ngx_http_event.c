...
do|do
block|{
comment|/* state_handlers return NGX_OK when whole header done */
name|rc
operator|=
operator|(
name|r
...
argument_list|)
expr_stmt|;
comment|/* If it's a pipelined request and a request line is not complete        then we need to copy it to the start of r->header_in hunk.        We need to copy it here only if the large client headers are enabled        otherwise a request line had already copied to start        of r->header_in hunk in ngx_http_set_keepalive() */
if|if
condition|(
...
argument_list|)
expr_stmt|;
comment|/* if large client headers is supported then            we need to copy request line */
name|r
operator|->
name|request_line
operator|.
...
argument_list|)
expr_stmt|;
comment|/* if large client header is supported then             we need to compact r->header_in hunk */
if|if
condition|(
name|ngx_http_large_client_header
operator|&&
...
argument_list|)
expr_stmt|;
comment|/* if large client headers is supported then        we need to copy header name and value */
name|h
operator|->
name|key
operator|.
...
condition|)
block|{
comment|/* clients that support pipelined requests (Mozilla 1.x, Opera 6.x)            are rare now so this copy should also be rare */
if|if
condition|(
operator|!
name|ngx_http_large_client_header
