...
argument|_                           r->header_in->last - r->header_in->pos
argument_list|)
empty_stmt|;
block|}
else|else
...
argument_list|)
expr_stmt|;
comment|/* if large client headers are enabled then        we need to copy header name and value */
name|h
operator|->
name|key
operator|.
...
condition|)
block|{
comment|/* We do not know here whether pipelined request is complete            so if large client headers are not enabled            we need to copy the data to the start of c->buffer.            This copy should be rare because clients that support            pipelined requests (Mozilla 1.x, Opera 6.x) are still rare */
if|if
condition|(
operator|!
name|ngx_http_large_client_header
