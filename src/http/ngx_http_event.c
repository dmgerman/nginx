...
do|do
block|{
comment|/* state_handlers return NGX_OK when the whole header done */
name|rc
operator|=
operator|(
name|r
...
name|connection
expr_stmt|;
comment|/* a request line has been parsed successfully */
if|if
condition|(
name|rc
operator|==
...
argument_list|)
expr_stmt|;
comment|/* if the large client headers is enabled then            we need to copy a request line */
name|r
operator|->
name|request_line
operator|.
...
expr_stmt|;
block|}
comment|/* if we need to parse the headers then return NGX_AGAIN            becuase of HTTP/0.9 has no headers so return NGX_OK */
if|if
condition|(
name|r
operator|->
...
name|NGX_AGAIN
return|;
comment|/* the whole header has been parsed successfully */
block|}
if|else if
condition|(
name|rc
...
condition|)
block|{
comment|/* if the large client header is enabled then                 we need to compact r->header_in hunk */
if|if
condition|(
name|ngx_http_large_client_header
condition|)
...
argument_list|)
expr_stmt|;
comment|/* if large client header is enabled then        we need to copy header name and value */
name|h
operator|->
name|key
operator|.
...
name|action
operator|=
literal|"processing client request"
expr_stmt|;
name|rc
operator|=
...
condition|)
block|{
comment|/* STUB: timeouts should be reworked */
if|if
condition|(
name|r
operator|->
...
condition|)
block|{
comment|/* We do not know here whether pipelined request is complete            so we need to copy it to the start of c->buffer if the large            client header is not enabled. This copy should be rare            because clients that support pipelined requests (Mozilla 1.x,            Opera 6.x) are still rare */
if|if
condition|(
operator|!
name|ngx_http_large_client_header
...
name|NGX_DONE
return|;
comment|/* TODO: MSIE closes keepalive connection with ECONNRESET              so we need to handle here this error              1) in INFO (not ERR) level, 2) with time elapsed */
name|n
operator|=
name|ngx_event_recv
argument_list|(
...
begin_function
DECL|function|ngx_http_header_parse_error (ngx_http_request_t * r,int parse_err,int err)
specifier|static
name|int
name|ngx_http_header_parse_error
parameter_list|(
