...
directive|if
literal|0
block|{ ngx_string("User-Agent"), offsetof(ngx_http_headers_in_t, user_agent) },
endif|#
directive|endif
block|{
...
begin_comment
unit|int ngx_http_discard_body(ngx_http_request_t *r) {     ngx_event_t  *ev;      ev = r->connection->read;      ngx_log_debug(r->connection->log, "set discard body");      if (ev->timer_set) {         ngx_del_timer(ev);         ev->timer_set = 0;     }      if (r->headers_in.content_length_n) {         ev->event_handler = ngx_http_read_discarded_body;
