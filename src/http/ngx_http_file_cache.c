...
begin_function
DECL|function|ngx_http_file_cache_get (ngx_http_request_t * r,ngx_http_cache_ctx_t * ctx)
name|ngx_int_t
...
literal|0
argument_list|,
literal|"file cache key: %V, md5: %s"
argument_list|,
operator|&
name|ctx
operator|->
...
begin_function
DECL|function|ngx_http_file_cache_open (ngx_http_cache_t * c)
name|ngx_int_t
name|ngx_http_file_cache_open
parameter_list|(
name|ngx_http_cache_t
...
literal|0
comment|/* TODO */
block_content|if (c->key_len&& h->key_len != c->key_len)  {          ngx_strncmp(h->key, c->key_data, h->key_len) != 0))          h->key[h->key_len] = '\0';         ngx_log_error(NGX_LOG_ALERT, c->log, 0,                           "md5 collision: \"%s\" and \"%s\"",                           h->key, c->key.data);         return NGX_DECLINED;     }
endif|#
directive|endif
name|c
operator|->
...
begin_if
unit|int ngx_http_cache_update_file(ngx_http_request_t *r, ngx_http_cache_ctx_t *ctx,                                ngx_str_t *temp_file) {     int        retry;     ngx_err_t  err;      retry = 0;      for ( ;; ) {         if (ngx_rename_file(temp_file->data, ctx->file.name.data) == NGX_OK) {             return NGX_OK;         }          err = ngx_errno;
if|#
directive|if
operator|(
name|NGX_WIN32
...
begin_function
DECL|function|ngx_http_cache_cleaner_handler (ngx_gc_t * gc,ngx_str_t * name,ngx_dir_t * dir)
name|ngx_int_t
