...
specifier|static
name|ngx_int_t
name|ngx_http_copy_pipelined_header
parameter_list|(
name|ngx_http_request_t
modifier|*
name|r
...
goto|;
block|}
comment|/*      * set by ngx_pcalloc():      *      *     rb->temp_file = NULL;      *     rb->bufs = NULL;      *     rb->buf = NULL;      *     rb->free = NULL;      *     rb->busy = NULL;      *     rb->chunked = NULL;      *     rb->received = 0;      *     rb->filter_need_buffering = 0;      *     rb->last_sent = 0;      *     rb->last_saved = 0;      */
name|rb
operator|->
name|rest
operator|=
...
begin_function
name|ngx_int_t
DECL|function|ngx_http_read_unbuffered_request_body (ngx_http_request_t * r)
...
specifier|static
name|ngx_int_t
DECL|function|ngx_http_copy_pipelined_header (ngx_http_request_t * r,ngx_buf_t * buf)
name|ngx_http_copy_pipelined_header
parameter_list|(
name|ngx_http_request_t
modifier|*
...
literal|0
argument_list|,
literal|"http body pipelined header: %uz"
argument_list|,
name|n
argument_list|)
expr_stmt|;
comment|/*      * if there is a pipelined request in the client body buffer,      * copy it to the r->header_in buffer if there is enough room,      * or allocate a large client header buffer      */
if|if
condition|(
name|n
operator|>
...
literal|0
argument_list|,
literal|"too large pipelined header after reading body"
argument_list|)
expr_stmt|;
return|return
name|NGX_ERROR
...
directive|if
literal|0
block_content|ngx_log_debug7(NGX_LOG_DEBUG_EVENT, r->connection->log, 0,                        "http body old buf t:%d f:%d %p, pos %p, size: %z "                        "file: %O, size: %O",                        cl->buf->temporary, cl->buf->in_file,                        cl->buf->start, cl->buf->pos,                        cl->buf->last - cl->buf->pos,                        cl->buf->file_pos,                        cl->buf->file_last - cl->buf->file_pos);
endif|#
directive|endif
name|ll
operator|=
...
literal|0
argument_list|,
literal|"duplicate last buf in save filter"
argument_list|)
expr_stmt|;
operator|*
name|ll
