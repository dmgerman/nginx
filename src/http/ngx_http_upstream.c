...
specifier|static
name|ngx_int_t
name|ngx_http_upstream_conditional_copy_header_line
parameter_list|(
name|ngx_http_request_t
modifier|*
name|r
...
name|NULL
expr_stmt|;
comment|/* reinit u->header_in buffer */
if|#
directive|if
literal|0
block_content|if (u->cache) {         u->header_in.pos = u->header_in.start + u->cache->ctx.header_size;         u->header_in.last = u->header_in.pos;      } else {         u->header_in.pos = u->header_in.start;         u->header_in.last = u->header_in.start;     }
else|#
directive|else
name|u
operator|->
...
directive|if
literal|0
block_content|if (u->cache) {             u->header_in.pos += u->cache->ctx.header_size;             u->header_in.last = u->header_in.pos;         }
endif|#
directive|endif
block|}
name|n
...
directive|if
literal|0
block_content|if (u->cache&& u->cache->ctx.file.fd != NGX_INVALID_FILE) {         if (ngx_close_file(u->cache->ctx.file.fd) == NGX_FILE_ERROR) {             ngx_log_error(NGX_LOG_ALERT, r->connection->log, ngx_errno,                           ngx_close_file_n " \"%s\" failed",                           u->cache->ctx.file.name.data);         }     }      if (u->cachable) {         header = (ngx_http_cache_header_t *) u->header_in->start;          header->expires = u->cache->ctx.expires;         header->last_modified = u->cache->ctx.last_modified;         header->date = u->cache->ctx.date;         header->length = r->headers_out.content_length_n;         u->cache->ctx.length = r->headers_out.content_length_n;          header->key_len = u->cache->ctx.key0.len;         ngx_memcpy(&header->key, u->cache->ctx.key0.data, header->key_len);         header->key[header->key_len] = LF;     }
endif|#
directive|endif
name|p
operator|=
...
literal|1
expr_stmt|;
comment|/*      * event_pipe would do u->header_in.last += p->preread_size      * as though these bytes were read      */
name|u
operator|->
name|header_in
operator|.
...
specifier|static
name|ngx_int_t
DECL|function|ngx_http_upstream_conditional_copy_header_line (ngx_http_request_t * r,ngx_table_elt_t * h,ngx_uint_t offset)
name|ngx_http_upstream_conditional_copy_header_line
parameter_list|(
name|ngx_http_request_t
modifier|*
...
name|u_char
modifier|*
DECL|function|ngx_http_upstream_log_error (ngx_http_request_t * r,u_char * buf,size_t len)
name|ngx_http_upstream_log_error
parameter_list|(
name|ngx_http_request_t
modifier|*
...
literal|0
argument_list|,
literal|"http upstream headers_in hash size: %ui, "
