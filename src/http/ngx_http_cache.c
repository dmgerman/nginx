...
begin_comment
unit|static ngx_http_module_t  ngx_http_cache_module_ctx = {     NULL,
comment|/* pre conf */
end_comment
...
begin_comment
unit|};   ngx_module_t  ngx_http_cache_module = {     NGX_MODULE,&ngx_http_cache_module_ctx,
...
begin_function
DECL|function|ngx_http_cache_create (ngx_http_request_t * r)
specifier|static
name|ngx_int_t
name|ngx_http_cache_create
parameter_list|(
...
return|;
block|}
comment|/* preallocate the primary key */
if|if
condition|(
operator|!
operator|(
...
name|NULL
expr_stmt|;
comment|/*      * we use offsetof() because sizeof() pads the struct size to the int size      */
name|r
operator|->
name|cache
operator|->
...
begin_function
DECL|function|ngx_http_cache_get (ngx_http_request_t * r,ngx_http_cache_ctx_t * ctx)
name|ngx_int_t
...
directive|if
literal|0
block_content|if (ctx->memory) {         ngx_http_memory_cache_get(r, ctx);     }
endif|#
directive|endif
if|if
condition|(
...
begin_if
unit|ngx_http_cache_t *ngx_http_cache_get(ngx_http_cache_hash_t *hash,                                      ngx_http_cleanup_t *cleanup,                                      ngx_str_t *key, uint32_t *crc) {     ngx_uint_t         i;     ngx_http_cache_t  *c;      *crc = ngx_crc(key->data, key->len);      c = hash->elts + *crc % hash->hash * hash->nelts;      if (ngx_mutex_lock(&hash->mutex) == NGX_ERROR) {         return (void *) NGX_ERROR;     }      for (i = 0; i< hash->nelts; i++) {         if (c[i].crc == *crc&& c[i].key.len == key->len&& ngx_rstrncmp(c[i].key.data, key->data, key->len) == 0)         {
if|#
directive|if
literal|0
end_if
...
begin_endif
unit|if (c[i].expired) {                 ngx_mutex_unlock(&hash->mutex);                 return (void *) NGX_AGAIN;             }
endif|#
directive|endif
end_endif

begin_comment
unit|c[i].refs++;              if ((!(c[i].notify&& (ngx_event_flags& NGX_USE_KQUEUE_EVENT)))&& (ngx_cached_time - c[i].updated>= hash->update))             {                 c[i].expired = 1;             }              ngx_mutex_unlock(&hash->mutex);              if (cleanup) {                 cleanup->data.cache.hash = hash;                 cleanup->data.cache.cache =&c[i];                 cleanup->valid = 1;                 cleanup->cache = 1;             }              return&c[i];         }     }      ngx_mutex_unlock(&hash->mutex);      return NULL; }   ngx_http_cache_t *ngx_http_cache_alloc(ngx_http_cache_hash_t *hash,                                        ngx_http_cache_t *cache,                                        ngx_http_cleanup_t *cleanup,                                        ngx_str_t *key, uint32_t crc,                                        ngx_str_t *value, ngx_log_t *log) {     time_t             old;     ngx_uint_t         i;     ngx_http_cache_t  *c;      old = ngx_cached_time + 1;      c = hash->elts + crc % hash->hash * hash->nelts;      if (ngx_mutex_lock(&hash->mutex) == NGX_ERROR) {         return (void *) NGX_ERROR;     }      if (cache == NULL) {
comment|/* allocate a new entry */
end_comment

begin_comment
unit|for (i = 0; i< hash->nelts; i++) {             if (c[i].refs> 0) {
comment|/* a busy entry */
end_comment

begin_comment
unit|continue;             }              if (c[i].key.len == 0) {
comment|/* a free entry is found */
end_comment

begin_comment
unit|cache =&c[i];                 break;             }
comment|/* looking for the oldest cache entry */
end_comment

begin_comment
unit|if (old> c[i].accessed) {                  old = c[i].accessed;                 cache =&c[i];             }         }          if (cache == NULL) {             ngx_mutex_unlock(&hash->mutex);             return NULL;         }          ngx_http_cache_free(cache, key, value, log);          if (cache->key.data == NULL) {             cache->key.data = ngx_alloc(key->len, log);             if (cache->key.data == NULL) {                 ngx_http_cache_free(cache, NULL, NULL, log);                 ngx_mutex_unlock(&hash->mutex);                 return NULL;             }         }          cache->key.len = key->len;         ngx_memcpy(cache->key.data, key->data, key->len);      } else if (value) {         ngx_http_cache_free(cache, key, value, log);     }      if (value) {         if (cache->data.value.data == NULL) {             cache->data.value.data = ngx_alloc(value->len, log);             if (cache->data.value.data == NULL) {                 ngx_http_cache_free(cache, NULL, NULL, log);                 ngx_mutex_unlock(&hash->mutex);                 return NULL;             }         }          cache->data.value.len = value->len;         ngx_memcpy(cache->data.value.data, value->data, value->len);     }      cache->crc = crc;     cache->key.len = key->len;      cache->refs = 1;     cache->count = 0;      cache->deleted = 0;     cache->expired = 0;     cache->memory = 0;     cache->mmap = 0;     cache->notify = 0;      if (cleanup) {         cleanup->data.cache.hash = hash;         cleanup->data.cache.cache = cache;         cleanup->valid = 1;         cleanup->cache = 1;     }      ngx_mutex_unlock(&hash->mutex);      return cache; }   void ngx_http_cache_free(ngx_http_cache_t *cache,                          ngx_str_t *key, ngx_str_t *value, ngx_log_t *log) {     if (cache->memory) {         if (cache->data.value.data&& (value == NULL || value->len> cache->data.value.len))         {             ngx_free(cache->data.value.data);             cache->data.value.data = NULL;         }     }
comment|/* TODO: mmap */
end_comment

begin_if
unit|cache->data.value.len = 0;      if (cache->fd != NGX_INVALID_FILE) {          ngx_log_debug1(NGX_LOG_DEBUG_HTTP, log, 0,                        "http cache close fd: %d", cache->fd);          if (ngx_close_file(cache->fd) == NGX_FILE_ERROR) {             ngx_log_error(NGX_LOG_ALERT, log, ngx_errno,                           ngx_close_file_n " \"%s\" failed",                           cache->key.data);         }          cache->fd = NGX_INVALID_FILE;     }      if (cache->key.data&& (key == NULL || key->len> cache->key.len)) {         ngx_free(cache->key.data);         cache->key.data = NULL;     }      cache->key.len = 0;      cache->refs = 0; }   void ngx_http_cache_lock(ngx_http_cache_hash_t *hash, ngx_http_cache_t *cache) {     if (ngx_mutex_lock(&hash->mutex) == NGX_ERROR) {         return;     } }   void ngx_http_cache_unlock(ngx_http_cache_hash_t *hash,                            ngx_http_cache_t *cache, ngx_log_t *log) {     if (ngx_mutex_lock(&hash->mutex) == NGX_ERROR) {         return;     }      cache->refs--;      if (cache->refs == 0&& cache->deleted) {         ngx_http_cache_free(cache, NULL, NULL, log);     }      ngx_mutex_unlock(&hash->mutex); }
if|#
directive|if
literal|0
end_if
