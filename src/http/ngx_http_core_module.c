...
begin_if
unit|static ngx_int_t ngx_http_core_init_process(ngx_cycle_t *cycle) {     ngx_uint_t                   i;     ngx_http_core_srv_conf_t   **cscfp;     ngx_http_core_main_conf_t   *cmcf;      cmcf = ngx_http_cycle_get_module_main_conf(cycle, ngx_http_core_module);
if|#
directive|if
literal|0
end_if
...
begin_endif
unit|cscfp = cmcf->servers.elts;      for (i = 0; i< cmcf->servers.nelts; i++) {         if (cscfp[i]->recv == NULL) {             cscfp[i]->recv = ngx_io.recv;             cscfp[i]->send_chain = ngx_io.send_chain;         }     }      return NGX_OK; }
endif|#
directive|endif
end_endif
...
begin_function
DECL|function|ngx_set_keepalive (ngx_conf_t * cf,ngx_command_t * cmd,void * conf)
specifier|static
name|char
modifier|*
name|ngx_set_keepalive
