...
begin_typedef
DECL|struct|__anon29ac82080108
typedef|typedef
struct|struct
block|{
DECL|member|dummy
...
argument_list|)
expr_stmt|;
comment|/* TODO: read a client's body */
name|chain
operator|=
name|ngx_http_proxy_create_request
argument_list|(
...
condition|)
block|{
comment|/* Here is the race condition                when the upstreams are shared between threads or processes                but it should not be serious */
name|p
operator|->
name|cur_upstream
operator|=
...
begin_if
unit|static int ngx_http_proxy_read_upstream_body(ngx_event_t *rev) {     int                    rc, n, size, eof;     ngx_hunk_t            *h;     ngx_chain_t           *chain, *ce, *tce;     ngx_event_t           *wev;     ngx_connection_t      *c;     ngx_http_request_t    *r;     ngx_http_proxy_ctx_t  *p;      c = (ngx_connection_t *) rev->data;     r = (ngx_http_request_t *) c->data;     p = (ngx_http_proxy_ctx_t *)                          ngx_http_get_module_ctx(r, ngx_http_proxy_module_ctx);      eof = 0;      for ( ;; ) {
if|#
directive|if
operator|(
name|USE_KQUEUE
...
begin_elif
unit|if (ev->eof&& ev->available == 0) {             eof = 1;             break;         }
elif|#
directive|elif
operator|(
name|HAVE_KQUEUE0
...
begin_endif
unit|if (ngx_event_type == NGX_HAVE_KQUEUE_EVENT&& ev->eof&& ev->available == 0)         {             eof = 1;             break;         }
endif|#
directive|endif
end_endif
