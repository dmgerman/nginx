...
condition|)
block|{
comment|/*          * we need to disable the use of sendfile() if we use cyclic temp file          * because the writing a new data can interfere with sendfile          * that uses the same kernel file pages          */
name|ep
operator|->
name|cyclic_temp_file
operator|=
