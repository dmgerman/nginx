...
begin_endif
unit|static void ngx_http_proxy_cache_busy_lock(ngx_http_proxy_ctx_t *p) {     rc = ngx_http_busy_lock(p->lcf->busy_lock, p->cache->ctx.md5);      if (rc == NGX_OK) {         ngx_http_proxy_request_upstream(p);     }      if (rc == NGX_AGAIN) {         if (p->busy_lock_time) {             ngx_add_timer(p->request->connection->read, 1000);             return;         }     }      rc == NGX_ERROR     check waitn }
endif|#
directive|endif
end_endif
