...
begin_typedef
DECL|struct|__anon28f8e4f90108
typedef|typedef
struct|struct
block|{
DECL|member|enable
...
begin_typedef
DECL|struct|__anon28f8e4f90208
typedef|typedef
struct|struct
block|{
DECL|member|in
...
block|}
block|}
comment|/*          * We preallocate a memory for zlib in one buffer (200K-400K), this          * decreases a number of malloc() and free() calls and also probably          * decreases a number of syscalls (sbrk() and so on).          * Besides we free this memory as soon as the gzipping will complete          * and do not wait while a whole response will be sent to a client.          *          * 8K is for zlib deflate_state, it takes          *  *) 5816 bytes on i386 and sparc64 (32-bit mode)          *  *) 5920 bytes on amd64 and sparc64          */
name|ctx
operator|->
name|allocated
operator|=
...
control|)
block|{
comment|/* does zlib need a new data ? */
if|if
condition|(
name|ctx
operator|->
...
argument_list|)
expr_stmt|;
comment|/*          * we do not check NGX_AGAIN here because the downstream filters          * may free some buffers and zlib may compress some data into them          */
if|if
condition|(
name|last
operator|==
