begin_unit|revision:1.0.0;language:C;cregit-version:0.0.1
begin_include
include|#
directive|include
file|<ngx_config.h>
end_include

begin_include
include|#
directive|include
file|<ngx_core.h>
end_include

begin_include
include|#
directive|include
file|<ngx_http.h>
end_include

begin_include
include|#
directive|include
file|<zlib.h>
end_include

begin_typedef
DECL|struct|__anon29e9c3600108
typedef|typedef
struct|struct
block|{
DECL|member|enable
name|ngx_flag_t
name|enable
decl_stmt|;
DECL|member|no_buffer
name|ngx_flag_t
name|no_buffer
decl_stmt|;
DECL|member|bufs
name|ngx_bufs_t
name|bufs
decl_stmt|;
DECL|member|http_version
name|ngx_uint_t
name|http_version
decl_stmt|;
DECL|member|proxied
name|ngx_uint_t
name|proxied
decl_stmt|;
DECL|member|level
name|int
name|level
decl_stmt|;
DECL|member|wbits
name|int
name|wbits
decl_stmt|;
DECL|member|memlevel
name|int
name|memlevel
decl_stmt|;
DECL|typedef|ngx_http_gzip_conf_t
block|}
name|ngx_http_gzip_conf_t
typedef|;
end_typedef

begin_enum
DECL|enum|__anon29e9c3600203
enum|enum
block|{
DECL|enumerator|NGX_HTTP_GZIP_PROXIED_OFF
name|NGX_HTTP_GZIP_PROXIED_OFF
init|=
literal|0
block|,
DECL|enumerator|NGX_HTTP_GZIP_PROXIED_NOCACHABLE
name|NGX_HTTP_GZIP_PROXIED_NOCACHABLE
block|,
DECL|enumerator|NGX_HTTP_GZIP_PROXIED_POOR_CACHABLE
name|NGX_HTTP_GZIP_PROXIED_POOR_CACHABLE
block|,
DECL|enumerator|NGX_HTTP_GZIP_PROXIED_ON
name|NGX_HTTP_GZIP_PROXIED_ON
block|}
enum|;
end_enum

begin_typedef
DECL|struct|__anon29e9c3600308
typedef|typedef
struct|struct
block|{
DECL|member|in
name|ngx_chain_t
modifier|*
name|in
decl_stmt|;
DECL|member|free
name|ngx_chain_t
modifier|*
name|free
decl_stmt|;
DECL|member|busy
name|ngx_chain_t
modifier|*
name|busy
decl_stmt|;
DECL|member|out
name|ngx_chain_t
modifier|*
name|out
decl_stmt|;
DECL|member|last_out
name|ngx_chain_t
modifier|*
modifier|*
name|last_out
decl_stmt|;
DECL|member|in_hunk
name|ngx_hunk_t
modifier|*
name|in_hunk
decl_stmt|;
DECL|member|out_hunk
name|ngx_hunk_t
modifier|*
name|out_hunk
decl_stmt|;
DECL|member|hunks
name|int
name|hunks
decl_stmt|;
DECL|member|length
name|off_t
name|length
decl_stmt|;
DECL|member|preallocated
name|void
modifier|*
name|preallocated
decl_stmt|;
DECL|member|free_mem
name|char
modifier|*
name|free_mem
decl_stmt|;
DECL|member|allocated
name|ngx_uint_t
name|allocated
decl_stmt|;
DECL|member|flush
name|unsigned
name|flush
range|:
literal|4
decl_stmt|;
DECL|member|redo
name|unsigned
name|redo
range|:
literal|1
decl_stmt|;
DECL|member|done
name|unsigned
name|done
range|:
literal|1
decl_stmt|;
DECL|member|zin
name|size_t
name|zin
decl_stmt|;
DECL|member|zout
name|size_t
name|zout
decl_stmt|;
DECL|member|crc32
name|uint32_t
name|crc32
decl_stmt|;
DECL|member|zstream
name|z_stream
name|zstream
decl_stmt|;
DECL|member|request
name|ngx_http_request_t
modifier|*
name|request
decl_stmt|;
DECL|typedef|ngx_http_gzip_ctx_t
block|}
name|ngx_http_gzip_ctx_t
typedef|;
end_typedef

begin_function_decl
specifier|static
name|void
modifier|*
name|ngx_http_gzip_filter_alloc
parameter_list|(
name|void
modifier|*
name|opaque
parameter_list|,
name|u_int
name|items
parameter_list|,
name|u_int
name|size
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
name|ngx_http_gzip_filter_free
parameter_list|(
name|void
modifier|*
name|opaque
parameter_list|,
name|void
modifier|*
name|address
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
name|ngx_inline
specifier|static
name|int
name|ngx_http_gzip_error
parameter_list|(
name|ngx_http_gzip_ctx_t
modifier|*
name|ctx
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|u_char
modifier|*
name|ngx_http_gzip_log_ratio
parameter_list|(
name|ngx_http_request_t
modifier|*
name|r
parameter_list|,
name|u_char
modifier|*
name|buf
parameter_list|,
name|uintptr_t
name|data
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|ngx_http_gzip_pre_conf
parameter_list|(
name|ngx_conf_t
modifier|*
name|cf
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|int
name|ngx_http_gzip_filter_init
parameter_list|(
name|ngx_cycle_t
modifier|*
name|cycle
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|void
modifier|*
name|ngx_http_gzip_create_conf
parameter_list|(
name|ngx_conf_t
modifier|*
name|cf
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|char
modifier|*
name|ngx_http_gzip_merge_conf
parameter_list|(
name|ngx_conf_t
modifier|*
name|cf
parameter_list|,
name|void
modifier|*
name|parent
parameter_list|,
name|void
modifier|*
name|child
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|char
modifier|*
name|ngx_http_gzip_set_window
parameter_list|(
name|ngx_conf_t
modifier|*
name|cf
parameter_list|,
name|void
modifier|*
name|post
parameter_list|,
name|void
modifier|*
name|data
parameter_list|)
function_decl|;
end_function_decl

begin_function_decl
specifier|static
name|char
modifier|*
name|ngx_http_gzip_set_hash
parameter_list|(
name|ngx_conf_t
modifier|*
name|cf
parameter_list|,
name|void
modifier|*
name|post
parameter_list|,
name|void
modifier|*
name|data
parameter_list|)
function_decl|;
end_function_decl

begin_decl_stmt
DECL|variable|ngx_http_gzip_comp_level_bounds
specifier|static
name|ngx_conf_num_bounds_t
name|ngx_http_gzip_comp_level_bounds
init|=
block|{
name|ngx_conf_check_num_bounds
block|,
literal|1
block|,
literal|9
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
DECL|variable|ngx_http_gzip_set_window_p
specifier|static
name|ngx_conf_post_handler_pt
name|ngx_http_gzip_set_window_p
init|=
name|ngx_http_gzip_set_window
decl_stmt|;
end_decl_stmt

begin_decl_stmt
DECL|variable|ngx_http_gzip_set_hash_p
specifier|static
name|ngx_conf_post_handler_pt
name|ngx_http_gzip_set_hash_p
init|=
name|ngx_http_gzip_set_hash
decl_stmt|;
end_decl_stmt

begin_decl_stmt
DECL|variable|ngx_http_gzip_http_version
specifier|static
name|ngx_conf_enum_t
name|ngx_http_gzip_http_version
index|[]
init|=
block|{
block|{
name|ngx_string
argument_list|(
literal|"1.0"
argument_list|)
block|,
name|NGX_HTTP_VERSION_10
block|}
block|,
block|{
name|ngx_string
argument_list|(
literal|"1.1"
argument_list|)
block|,
name|NGX_HTTP_VERSION_11
block|}
block|,
block|{
name|ngx_null_string
block|,
literal|0
block|}
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
DECL|variable|ngx_http_gzip_proxied
specifier|static
name|ngx_conf_enum_t
name|ngx_http_gzip_proxied
index|[]
init|=
block|{
block|{
name|ngx_string
argument_list|(
literal|"off"
argument_list|)
block|,
name|NGX_HTTP_GZIP_PROXIED_OFF
block|}
block|,
if|#
directive|if
literal|0
block|{ ngx_string("nocachable"), NGX_HTTP_GZIP_PROXIED_NOCACHABLE },     { ngx_string("poor_cachable"), NGX_HTTP_GZIP_PROXIED_POOR_CACHABLE },
endif|#
directive|endif
block|{
name|ngx_string
argument_list|(
literal|"on"
argument_list|)
block|,
name|NGX_HTTP_GZIP_PROXIED_ON
block|}
block|,
block|{
name|ngx_null_string
block|,
literal|0
block|}
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
DECL|variable|ngx_http_gzip_filter_commands
specifier|static
name|ngx_command_t
name|ngx_http_gzip_filter_commands
index|[]
init|=
block|{
block|{
name|ngx_string
argument_list|(
literal|"gzip"
argument_list|)
block|,
name|NGX_HTTP_MAIN_CONF
operator||
name|NGX_HTTP_SRV_CONF
operator||
name|NGX_HTTP_LOC_CONF
operator||
name|NGX_CONF_FLAG
block|,
name|ngx_conf_set_flag_slot
block|,
name|NGX_HTTP_LOC_CONF_OFFSET
block|,
name|offsetof
argument_list|(
name|ngx_http_gzip_conf_t
argument_list|,
name|enable
argument_list|)
block|,
name|NULL
block|}
block|,
block|{
name|ngx_string
argument_list|(
literal|"gzip_buffers"
argument_list|)
block|,
name|NGX_HTTP_MAIN_CONF
operator||
name|NGX_HTTP_SRV_CONF
operator||
name|NGX_HTTP_LOC_CONF
operator||
name|NGX_CONF_TAKE2
block|,
name|ngx_conf_set_bufs_slot
block|,
name|NGX_HTTP_LOC_CONF_OFFSET
block|,
name|offsetof
argument_list|(
name|ngx_http_gzip_conf_t
argument_list|,
name|bufs
argument_list|)
block|,
name|NULL
block|}
block|,
block|{
name|ngx_string
argument_list|(
literal|"gzip_comp_level"
argument_list|)
block|,
name|NGX_HTTP_MAIN_CONF
operator||
name|NGX_HTTP_SRV_CONF
operator||
name|NGX_HTTP_LOC_CONF
operator||
name|NGX_CONF_TAKE1
block|,
name|ngx_conf_set_num_slot
block|,
name|NGX_HTTP_LOC_CONF_OFFSET
block|,
name|offsetof
argument_list|(
name|ngx_http_gzip_conf_t
argument_list|,
name|level
argument_list|)
block|,
operator|&
name|ngx_http_gzip_comp_level_bounds
block|}
block|,
block|{
name|ngx_string
argument_list|(
literal|"gzip_window"
argument_list|)
block|,
name|NGX_HTTP_MAIN_CONF
operator||
name|NGX_HTTP_SRV_CONF
operator||
name|NGX_HTTP_LOC_CONF
operator||
name|NGX_CONF_TAKE1
block|,
name|ngx_conf_set_size_slot
block|,
name|NGX_HTTP_LOC_CONF_OFFSET
block|,
name|offsetof
argument_list|(
name|ngx_http_gzip_conf_t
argument_list|,
name|wbits
argument_list|)
block|,
operator|&
name|ngx_http_gzip_set_window_p
block|}
block|,
block|{
name|ngx_string
argument_list|(
literal|"gzip_hash"
argument_list|)
block|,
name|NGX_HTTP_MAIN_CONF
operator||
name|NGX_HTTP_SRV_CONF
operator||
name|NGX_HTTP_LOC_CONF
operator||
name|NGX_CONF_TAKE1
block|,
name|ngx_conf_set_size_slot
block|,
name|NGX_HTTP_LOC_CONF_OFFSET
block|,
name|offsetof
argument_list|(
name|ngx_http_gzip_conf_t
argument_list|,
name|memlevel
argument_list|)
block|,
operator|&
name|ngx_http_gzip_set_hash_p
block|}
block|,
block|{
name|ngx_string
argument_list|(
literal|"gzip_no_buffer"
argument_list|)
block|,
name|NGX_HTTP_MAIN_CONF
operator||
name|NGX_HTTP_SRV_CONF
operator||
name|NGX_HTTP_LOC_CONF
operator||
name|NGX_CONF_FLAG
block|,
name|ngx_conf_set_flag_slot
block|,
name|NGX_HTTP_LOC_CONF_OFFSET
block|,
name|offsetof
argument_list|(
name|ngx_http_gzip_conf_t
argument_list|,
name|no_buffer
argument_list|)
block|,
name|NULL
block|}
block|,
block|{
name|ngx_string
argument_list|(
literal|"gzip_http_version"
argument_list|)
block|,
name|NGX_HTTP_MAIN_CONF
operator||
name|NGX_HTTP_SRV_CONF
operator||
name|NGX_HTTP_LOC_CONF
operator||
name|NGX_CONF_ANY
block|,
name|ngx_conf_set_enum_slot
block|,
name|NGX_HTTP_LOC_CONF_OFFSET
block|,
name|offsetof
argument_list|(
name|ngx_http_gzip_conf_t
argument_list|,
name|http_version
argument_list|)
block|,
operator|&
name|ngx_http_gzip_http_version
block|}
block|,
block|{
name|ngx_string
argument_list|(
literal|"gzip_proxied"
argument_list|)
block|,
name|NGX_HTTP_MAIN_CONF
operator||
name|NGX_HTTP_SRV_CONF
operator||
name|NGX_HTTP_LOC_CONF
operator||
name|NGX_CONF_ANY
block|,
name|ngx_conf_set_enum_slot
block|,
name|NGX_HTTP_LOC_CONF_OFFSET
block|,
name|offsetof
argument_list|(
name|ngx_http_gzip_conf_t
argument_list|,
name|proxied
argument_list|)
block|,
operator|&
name|ngx_http_gzip_proxied
block|}
block|,
name|ngx_null_command
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
DECL|variable|ngx_http_gzip_filter_module_ctx
specifier|static
name|ngx_http_module_t
name|ngx_http_gzip_filter_module_ctx
init|=
block|{
name|ngx_http_gzip_pre_conf
block|,
comment|/* pre conf */
name|NULL
block|,
comment|/* create main configuration */
name|NULL
block|,
comment|/* init main configuration */
name|NULL
block|,
comment|/* create server configuration */
name|NULL
block|,
comment|/* merge server configuration */
name|ngx_http_gzip_create_conf
block|,
comment|/* create location configuration */
name|ngx_http_gzip_merge_conf
block|,
comment|/* merge location configuration */
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
DECL|variable|ngx_http_gzip_filter_module
name|ngx_module_t
name|ngx_http_gzip_filter_module
init|=
block|{
name|NGX_MODULE
block|,
operator|&
name|ngx_http_gzip_filter_module_ctx
block|,
comment|/* module context */
name|ngx_http_gzip_filter_commands
block|,
comment|/* module directives */
name|NGX_HTTP_MODULE
block|,
comment|/* module type */
name|ngx_http_gzip_filter_init
block|,
comment|/* init module */
name|NULL
comment|/* init child */
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
DECL|variable|ngx_http_gzip_log_fmt_ops
specifier|static
name|ngx_http_log_op_name_t
name|ngx_http_gzip_log_fmt_ops
index|[]
init|=
block|{
block|{
name|ngx_string
argument_list|(
literal|"gzip_ratio"
argument_list|)
block|,
name|NGX_INT32_LEN
operator|+
literal|3
block|,
name|ngx_http_gzip_log_ratio
block|}
block|,
block|{
name|ngx_null_string
block|,
literal|0
block|,
name|NULL
block|}
block|}
decl_stmt|;
end_decl_stmt

begin_decl_stmt
DECL|variable|gzheader
specifier|static
name|u_char
name|gzheader
index|[
literal|10
index|]
init|=
block|{
literal|0x1f
block|,
literal|0x8b
block|,
name|Z_DEFLATED
block|,
literal|0
block|,
literal|0
block|,
literal|0
block|,
literal|0
block|,
literal|0
block|,
literal|0
block|,
literal|3
block|}
decl_stmt|;
end_decl_stmt

begin_if
if|#
directive|if
operator|(
name|HAVE_LITTLE_ENDIAN
operator|)
end_if

begin_struct
DECL|struct|gztrailer
struct|struct
name|gztrailer
block|{
DECL|member|crc32
name|uint32_t
name|crc32
decl_stmt|;
DECL|member|zlen
name|uint32_t
name|zlen
decl_stmt|;
block|}
struct|;
end_struct

begin_else
else|#
directive|else
end_else

begin_comment
comment|/* HAVE_BIG_ENDIAN */
end_comment

begin_struct
DECL|struct|gztrailer
struct|struct
name|gztrailer
block|{
DECL|member|crc32
name|u_char
name|crc32
index|[
literal|4
index|]
decl_stmt|;
DECL|member|zlen
name|u_char
name|zlen
index|[
literal|4
index|]
decl_stmt|;
block|}
struct|;
end_struct

begin_endif
endif|#
directive|endif
end_endif

begin_decl_stmt
DECL|variable|ngx_http_next_header_filter
specifier|static
name|ngx_http_output_header_filter_pt
name|ngx_http_next_header_filter
decl_stmt|;
end_decl_stmt

begin_decl_stmt
DECL|variable|ngx_http_next_body_filter
specifier|static
name|ngx_http_output_body_filter_pt
name|ngx_http_next_body_filter
decl_stmt|;
end_decl_stmt

begin_function
DECL|function|ngx_http_gzip_header_filter (ngx_http_request_t * r)
specifier|static
name|int
name|ngx_http_gzip_header_filter
parameter_list|(
name|ngx_http_request_t
modifier|*
name|r
parameter_list|)
block|{
name|ngx_http_gzip_ctx_t
modifier|*
name|ctx
decl_stmt|;
name|ngx_http_gzip_conf_t
modifier|*
name|conf
decl_stmt|;
name|conf
operator|=
name|ngx_http_get_module_loc_conf
argument_list|(
name|r
argument_list|,
name|ngx_http_gzip_filter_module
argument_list|)
expr_stmt|;
if|if
condition|(
operator|!
name|conf
operator|->
name|enable
operator|||
name|r
operator|->
name|headers_out
operator|.
name|status
operator|!=
name|NGX_HTTP_OK
operator|||
name|r
operator|->
name|header_only
operator|||
name|r
operator|->
name|http_version
operator|<
name|conf
operator|->
name|http_version
operator|||
operator|(
name|r
operator|->
name|headers_out
operator|.
name|content_encoding
operator|&&
name|r
operator|->
name|headers_out
operator|.
name|content_encoding
operator|->
name|value
operator|.
name|len
operator|)
operator|||
name|r
operator|->
name|headers_in
operator|.
name|accept_encoding
operator|==
name|NULL
operator|||
name|ngx_strstr
argument_list|(
name|r
operator|->
name|headers_in
operator|.
name|accept_encoding
operator|->
name|value
operator|.
name|data
argument_list|,
literal|"gzip"
argument_list|)
operator|==
name|NULL
condition|)
block|{
return|return
name|ngx_http_next_header_filter
argument_list|(
name|r
argument_list|)
return|;
block|}
comment|/* TODO: "text/html" -> custom types */
if|if
condition|(
name|r
operator|->
name|headers_out
operator|.
name|content_type
operator|&&
name|ngx_strncasecmp
argument_list|(
name|r
operator|->
name|headers_out
operator|.
name|content_type
operator|->
name|value
operator|.
name|data
argument_list|,
literal|"text/html"
argument_list|,
literal|5
argument_list|)
operator|!=
literal|0
condition|)
block|{
return|return
name|ngx_http_next_header_filter
argument_list|(
name|r
argument_list|)
return|;
block|}
comment|/* TODO: proxied */
if|if
condition|(
name|r
operator|->
name|headers_in
operator|.
name|via
operator|&&
name|conf
operator|->
name|proxied
operator|==
name|NGX_HTTP_GZIP_PROXIED_OFF
condition|)
block|{
return|return
name|ngx_http_next_header_filter
argument_list|(
name|r
argument_list|)
return|;
block|}
comment|/*      * if the URL (without the "http://" prefix) is longer than 253 bytes      * then MSIE 4.x can not handle the compressed stream - it waits too long,      * hangs up or crashes      */
if|if
condition|(
name|r
operator|->
name|headers_in
operator|.
name|user_agent
operator|&&
name|r
operator|->
name|unparsed_uri
operator|.
name|len
operator|>
literal|200
operator|&&
name|ngx_strstr
argument_list|(
name|r
operator|->
name|headers_in
operator|.
name|user_agent
operator|->
name|value
operator|.
name|data
argument_list|,
literal|"MSIE 4"
argument_list|)
condition|)
block|{
return|return
name|ngx_http_next_header_filter
argument_list|(
name|r
argument_list|)
return|;
block|}
name|ngx_http_create_ctx
argument_list|(
name|r
argument_list|,
name|ctx
argument_list|,
name|ngx_http_gzip_filter_module
argument_list|,
sizeof|sizeof
argument_list|(
name|ngx_http_gzip_ctx_t
argument_list|)
argument_list|,
name|NGX_ERROR
argument_list|)
expr_stmt|;
name|ctx
operator|->
name|request
operator|=
name|r
expr_stmt|;
name|r
operator|->
name|headers_out
operator|.
name|content_encoding
operator|=
name|ngx_http_add_header
argument_list|(
operator|&
name|r
operator|->
name|headers_out
argument_list|,
name|ngx_http_headers_out
argument_list|)
expr_stmt|;
if|if
condition|(
name|r
operator|->
name|headers_out
operator|.
name|content_encoding
operator|==
name|NULL
condition|)
block|{
return|return
name|NGX_ERROR
return|;
block|}
name|r
operator|->
name|headers_out
operator|.
name|content_encoding
operator|->
name|key
operator|.
name|len
operator|=
sizeof|sizeof
argument_list|(
literal|"Content-Encoding"
argument_list|)
operator|-
literal|1
expr_stmt|;
name|r
operator|->
name|headers_out
operator|.
name|content_encoding
operator|->
name|key
operator|.
name|data
operator|=
operator|(
name|u_char
operator|*
operator|)
literal|"Content-Encoding"
expr_stmt|;
name|r
operator|->
name|headers_out
operator|.
name|content_encoding
operator|->
name|value
operator|.
name|len
operator|=
sizeof|sizeof
argument_list|(
literal|"gzip"
argument_list|)
operator|-
literal|1
expr_stmt|;
name|r
operator|->
name|headers_out
operator|.
name|content_encoding
operator|->
name|value
operator|.
name|data
operator|=
operator|(
name|u_char
operator|*
operator|)
literal|"gzip"
expr_stmt|;
name|ctx
operator|->
name|length
operator|=
name|r
operator|->
name|headers_out
operator|.
name|content_length_n
expr_stmt|;
name|r
operator|->
name|headers_out
operator|.
name|content_length_n
operator|=
operator|-
literal|1
expr_stmt|;
if|if
condition|(
name|r
operator|->
name|headers_out
operator|.
name|content_length
condition|)
block|{
name|r
operator|->
name|headers_out
operator|.
name|content_length
operator|->
name|key
operator|.
name|len
operator|=
literal|0
expr_stmt|;
name|r
operator|->
name|headers_out
operator|.
name|content_length
operator|=
name|NULL
expr_stmt|;
block|}
name|r
operator|->
name|filter
operator||=
name|NGX_HTTP_FILTER_NEED_IN_MEMORY
expr_stmt|;
return|return
name|ngx_http_next_header_filter
argument_list|(
name|r
argument_list|)
return|;
block|}
end_function

begin_function
DECL|function|ngx_http_gzip_body_filter (ngx_http_request_t * r,ngx_chain_t * in)
specifier|static
name|int
name|ngx_http_gzip_body_filter
parameter_list|(
name|ngx_http_request_t
modifier|*
name|r
parameter_list|,
name|ngx_chain_t
modifier|*
name|in
parameter_list|)
block|{
name|int
name|rc
decl_stmt|,
name|wbits
decl_stmt|,
name|memlevel
decl_stmt|,
name|last
decl_stmt|;
name|struct
name|gztrailer
modifier|*
name|trailer
decl_stmt|;
name|ngx_hunk_t
modifier|*
name|h
decl_stmt|;
name|ngx_chain_t
modifier|*
name|cl
decl_stmt|;
name|ngx_http_gzip_ctx_t
modifier|*
name|ctx
decl_stmt|;
name|ngx_http_gzip_conf_t
modifier|*
name|conf
decl_stmt|;
name|ctx
operator|=
name|ngx_http_get_module_ctx
argument_list|(
name|r
argument_list|,
name|ngx_http_gzip_filter_module
argument_list|)
expr_stmt|;
if|if
condition|(
name|ctx
operator|==
name|NULL
operator|||
name|ctx
operator|->
name|done
condition|)
block|{
return|return
name|ngx_http_next_body_filter
argument_list|(
name|r
argument_list|,
name|in
argument_list|)
return|;
block|}
name|conf
operator|=
name|ngx_http_get_module_loc_conf
argument_list|(
name|r
argument_list|,
name|ngx_http_gzip_filter_module
argument_list|)
expr_stmt|;
if|if
condition|(
name|ctx
operator|->
name|preallocated
operator|==
name|NULL
condition|)
block|{
name|wbits
operator|=
name|conf
operator|->
name|wbits
expr_stmt|;
name|memlevel
operator|=
name|conf
operator|->
name|memlevel
expr_stmt|;
if|if
condition|(
name|ctx
operator|->
name|length
operator|>
literal|0
condition|)
block|{
comment|/* the actual zlib window size is smaller by 262 bytes */
while|while
condition|(
name|ctx
operator|->
name|length
operator|<
operator|(
operator|(
literal|1
operator|<<
operator|(
name|wbits
operator|-
literal|1
operator|)
operator|)
operator|-
literal|262
operator|)
condition|)
block|{
name|wbits
operator|--
expr_stmt|;
name|memlevel
operator|--
expr_stmt|;
block|}
block|}
comment|/*          * We preallocate a memory for zlib in one hunk (200K-400K), this          * dicreases a number of malloc() and free() calls and also probably          * dicreases a number of syscalls.          * Besides we free() this memory as soon as the gzipping will complete          * and do not wait while a whole response will be sent to a client.          *          * 8K is for zlib deflate_state (~6K).          *          * TODO: 64-bit, round to PAGE_SIZE, autoconf of deflate_state size          */
name|ctx
operator|->
name|allocated
operator|=
literal|8192
operator|+
operator|(
literal|1
operator|<<
operator|(
name|wbits
operator|+
literal|2
operator|)
operator|)
operator|+
operator|(
literal|1
operator|<<
operator|(
name|memlevel
operator|+
literal|9
operator|)
operator|)
expr_stmt|;
name|ngx_test_null
argument_list|(
name|ctx
operator|->
name|preallocated
argument_list|,
name|ngx_palloc
argument_list|(
name|r
operator|->
name|pool
argument_list|,
name|ctx
operator|->
name|allocated
argument_list|)
argument_list|,
name|NGX_ERROR
argument_list|)
expr_stmt|;
name|ctx
operator|->
name|free_mem
operator|=
name|ctx
operator|->
name|preallocated
expr_stmt|;
name|ctx
operator|->
name|zstream
operator|.
name|zalloc
operator|=
name|ngx_http_gzip_filter_alloc
expr_stmt|;
name|ctx
operator|->
name|zstream
operator|.
name|zfree
operator|=
name|ngx_http_gzip_filter_free
expr_stmt|;
name|ctx
operator|->
name|zstream
operator|.
name|opaque
operator|=
name|ctx
expr_stmt|;
name|rc
operator|=
name|deflateInit2
argument_list|(
operator|&
name|ctx
operator|->
name|zstream
argument_list|,
name|conf
operator|->
name|level
argument_list|,
name|Z_DEFLATED
argument_list|,
operator|-
name|wbits
argument_list|,
name|memlevel
argument_list|,
name|Z_DEFAULT_STRATEGY
argument_list|)
expr_stmt|;
if|if
condition|(
name|rc
operator|!=
name|Z_OK
condition|)
block|{
name|ngx_log_error
argument_list|(
name|NGX_LOG_ALERT
argument_list|,
name|r
operator|->
name|connection
operator|->
name|log
argument_list|,
literal|0
argument_list|,
literal|"deflateInit2() failed: %d"
argument_list|,
name|rc
argument_list|)
expr_stmt|;
return|return
name|ngx_http_gzip_error
argument_list|(
name|ctx
argument_list|)
return|;
block|}
name|ngx_test_null
argument_list|(
name|h
argument_list|,
name|ngx_calloc_hunk
argument_list|(
name|r
operator|->
name|pool
argument_list|)
argument_list|,
name|ngx_http_gzip_error
argument_list|(
name|ctx
argument_list|)
argument_list|)
expr_stmt|;
name|h
operator|->
name|type
operator|=
name|NGX_HUNK_IN_MEMORY
operator||
name|NGX_HUNK_MEMORY
expr_stmt|;
name|h
operator|->
name|pos
operator|=
name|gzheader
expr_stmt|;
name|h
operator|->
name|last
operator|=
name|h
operator|->
name|pos
operator|+
literal|10
expr_stmt|;
name|ngx_alloc_link_and_set_hunk
argument_list|(
name|cl
argument_list|,
name|h
argument_list|,
name|r
operator|->
name|pool
argument_list|,
name|ngx_http_gzip_error
argument_list|(
name|ctx
argument_list|)
argument_list|)
expr_stmt|;
name|ctx
operator|->
name|out
operator|=
name|cl
expr_stmt|;
name|ctx
operator|->
name|last_out
operator|=
operator|&
name|cl
operator|->
name|next
expr_stmt|;
name|ctx
operator|->
name|crc32
operator|=
name|crc32
argument_list|(
literal|0L
argument_list|,
name|Z_NULL
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|ctx
operator|->
name|flush
operator|=
name|Z_NO_FLUSH
expr_stmt|;
block|}
if|if
condition|(
name|in
condition|)
block|{
if|if
condition|(
name|ngx_chain_add_copy
argument_list|(
name|r
operator|->
name|pool
argument_list|,
operator|&
name|ctx
operator|->
name|in
argument_list|,
name|in
argument_list|)
operator|==
name|NGX_ERROR
condition|)
block|{
return|return
name|ngx_http_gzip_error
argument_list|(
name|ctx
argument_list|)
return|;
block|}
block|}
name|last
operator|=
name|NGX_NONE
expr_stmt|;
for|for
control|(
init|;
condition|;
control|)
block|{
for|for
control|(
init|;
condition|;
control|)
block|{
comment|/* is there a data to gzip ? */
if|if
condition|(
name|ctx
operator|->
name|zstream
operator|.
name|avail_in
operator|==
literal|0
operator|&&
name|ctx
operator|->
name|flush
operator|==
name|Z_NO_FLUSH
operator|&&
operator|!
name|ctx
operator|->
name|redo
condition|)
block|{
if|if
condition|(
name|ctx
operator|->
name|in
operator|==
name|NULL
condition|)
block|{
break|break;
block|}
name|ctx
operator|->
name|in_hunk
operator|=
name|ctx
operator|->
name|in
operator|->
name|hunk
expr_stmt|;
name|ctx
operator|->
name|in
operator|=
name|ctx
operator|->
name|in
operator|->
name|next
expr_stmt|;
name|ctx
operator|->
name|zstream
operator|.
name|next_in
operator|=
operator|(
name|u_char
operator|*
operator|)
name|ctx
operator|->
name|in_hunk
operator|->
name|pos
expr_stmt|;
name|ctx
operator|->
name|zstream
operator|.
name|avail_in
operator|=
name|ctx
operator|->
name|in_hunk
operator|->
name|last
operator|-
name|ctx
operator|->
name|in_hunk
operator|->
name|pos
expr_stmt|;
if|if
condition|(
name|ctx
operator|->
name|in_hunk
operator|->
name|type
operator|&
name|NGX_HUNK_LAST
condition|)
block|{
name|ctx
operator|->
name|flush
operator|=
name|Z_FINISH
expr_stmt|;
block|}
if|else if
condition|(
name|ctx
operator|->
name|in_hunk
operator|->
name|type
operator|&
name|NGX_HUNK_FLUSH
condition|)
block|{
name|ctx
operator|->
name|flush
operator|=
name|Z_SYNC_FLUSH
expr_stmt|;
block|}
if|if
condition|(
name|ctx
operator|->
name|zstream
operator|.
name|avail_in
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|ctx
operator|->
name|flush
operator|==
name|Z_NO_FLUSH
condition|)
block|{
continue|continue;
block|}
block|}
else|else
block|{
name|ctx
operator|->
name|crc32
operator|=
name|crc32
argument_list|(
name|ctx
operator|->
name|crc32
argument_list|,
name|ctx
operator|->
name|zstream
operator|.
name|next_in
argument_list|,
name|ctx
operator|->
name|zstream
operator|.
name|avail_in
argument_list|)
expr_stmt|;
block|}
block|}
comment|/* is there a space for the gzipped data ? */
if|if
condition|(
name|ctx
operator|->
name|zstream
operator|.
name|avail_out
operator|==
literal|0
condition|)
block|{
if|if
condition|(
name|ctx
operator|->
name|free
condition|)
block|{
name|ctx
operator|->
name|out_hunk
operator|=
name|ctx
operator|->
name|free
operator|->
name|hunk
expr_stmt|;
name|ctx
operator|->
name|free
operator|=
name|ctx
operator|->
name|free
operator|->
name|next
expr_stmt|;
block|}
if|else if
condition|(
name|ctx
operator|->
name|hunks
operator|<
name|conf
operator|->
name|bufs
operator|.
name|num
condition|)
block|{
name|ngx_test_null
argument_list|(
name|ctx
operator|->
name|out_hunk
argument_list|,
name|ngx_create_temp_hunk
argument_list|(
name|r
operator|->
name|pool
argument_list|,
name|conf
operator|->
name|bufs
operator|.
name|size
argument_list|)
argument_list|,
name|ngx_http_gzip_error
argument_list|(
name|ctx
argument_list|)
argument_list|)
expr_stmt|;
name|ctx
operator|->
name|out_hunk
operator|->
name|tag
operator|=
operator|(
name|ngx_hunk_tag_t
operator|)
operator|&
name|ngx_http_gzip_filter_module
expr_stmt|;
name|ctx
operator|->
name|out_hunk
operator|->
name|type
operator||=
name|NGX_HUNK_RECYCLED
expr_stmt|;
name|ctx
operator|->
name|hunks
operator|++
expr_stmt|;
block|}
else|else
block|{
break|break;
block|}
name|ctx
operator|->
name|zstream
operator|.
name|next_out
operator|=
operator|(
name|u_char
operator|*
operator|)
name|ctx
operator|->
name|out_hunk
operator|->
name|pos
expr_stmt|;
name|ctx
operator|->
name|zstream
operator|.
name|avail_out
operator|=
name|conf
operator|->
name|bufs
operator|.
name|size
expr_stmt|;
block|}
name|ngx_log_debug5
argument_list|(
name|NGX_LOG_DEBUG_HTTP
argument_list|,
name|r
operator|->
name|connection
operator|->
name|log
argument_list|,
literal|0
argument_list|,
literal|"deflate in: ni:%X no:%X ai:%d ao:%d fl:%d"
argument_list|,
name|ctx
operator|->
name|zstream
operator|.
name|next_in
argument_list|,
name|ctx
operator|->
name|zstream
operator|.
name|next_out
argument_list|,
name|ctx
operator|->
name|zstream
operator|.
name|avail_in
argument_list|,
name|ctx
operator|->
name|zstream
operator|.
name|avail_out
argument_list|,
name|ctx
operator|->
name|flush
argument_list|)
expr_stmt|;
name|rc
operator|=
name|deflate
argument_list|(
operator|&
name|ctx
operator|->
name|zstream
argument_list|,
name|ctx
operator|->
name|flush
argument_list|)
expr_stmt|;
if|if
condition|(
name|rc
operator|!=
name|Z_OK
operator|&&
name|rc
operator|!=
name|Z_STREAM_END
condition|)
block|{
name|ngx_log_error
argument_list|(
name|NGX_LOG_ALERT
argument_list|,
name|r
operator|->
name|connection
operator|->
name|log
argument_list|,
literal|0
argument_list|,
literal|"deflate() failed: %d, %d"
argument_list|,
name|ctx
operator|->
name|flush
argument_list|,
name|rc
argument_list|)
expr_stmt|;
return|return
name|ngx_http_gzip_error
argument_list|(
name|ctx
argument_list|)
return|;
block|}
name|ngx_log_debug5
argument_list|(
name|NGX_LOG_DEBUG_HTTP
argument_list|,
name|r
operator|->
name|connection
operator|->
name|log
argument_list|,
literal|0
argument_list|,
literal|"deflate out: ni:%X no:%X ai:%d ao:%d rc:%d"
argument_list|,
name|ctx
operator|->
name|zstream
operator|.
name|next_in
argument_list|,
name|ctx
operator|->
name|zstream
operator|.
name|next_out
argument_list|,
name|ctx
operator|->
name|zstream
operator|.
name|avail_in
argument_list|,
name|ctx
operator|->
name|zstream
operator|.
name|avail_out
argument_list|,
name|rc
argument_list|)
expr_stmt|;
name|ctx
operator|->
name|in_hunk
operator|->
name|pos
operator|=
name|ctx
operator|->
name|zstream
operator|.
name|next_in
expr_stmt|;
name|ctx
operator|->
name|out_hunk
operator|->
name|last
operator|=
name|ctx
operator|->
name|zstream
operator|.
name|next_out
expr_stmt|;
if|if
condition|(
name|ctx
operator|->
name|zstream
operator|.
name|avail_out
operator|==
literal|0
condition|)
block|{
name|ngx_alloc_link_and_set_hunk
argument_list|(
name|cl
argument_list|,
name|ctx
operator|->
name|out_hunk
argument_list|,
name|r
operator|->
name|pool
argument_list|,
name|ngx_http_gzip_error
argument_list|(
name|ctx
argument_list|)
argument_list|)
expr_stmt|;
operator|*
name|ctx
operator|->
name|last_out
operator|=
name|cl
expr_stmt|;
name|ctx
operator|->
name|last_out
operator|=
operator|&
name|cl
operator|->
name|next
expr_stmt|;
name|ctx
operator|->
name|redo
operator|=
literal|1
expr_stmt|;
block|}
else|else
block|{
name|ctx
operator|->
name|redo
operator|=
literal|0
expr_stmt|;
if|if
condition|(
name|ctx
operator|->
name|flush
operator|==
name|Z_SYNC_FLUSH
condition|)
block|{
name|ctx
operator|->
name|out_hunk
operator|->
name|type
operator||=
name|NGX_HUNK_FLUSH
expr_stmt|;
name|ctx
operator|->
name|flush
operator|=
name|Z_NO_FLUSH
expr_stmt|;
name|ngx_alloc_link_and_set_hunk
argument_list|(
name|cl
argument_list|,
name|ctx
operator|->
name|out_hunk
argument_list|,
name|r
operator|->
name|pool
argument_list|,
name|ngx_http_gzip_error
argument_list|(
name|ctx
argument_list|)
argument_list|)
expr_stmt|;
operator|*
name|ctx
operator|->
name|last_out
operator|=
name|cl
expr_stmt|;
name|ctx
operator|->
name|last_out
operator|=
operator|&
name|cl
operator|->
name|next
expr_stmt|;
break|break;
block|}
if|else if
condition|(
name|ctx
operator|->
name|flush
operator|==
name|Z_FINISH
condition|)
block|{
comment|/* rc == Z_STREAM_END */
name|ctx
operator|->
name|zin
operator|=
name|ctx
operator|->
name|zstream
operator|.
name|total_in
expr_stmt|;
name|ctx
operator|->
name|zout
operator|=
literal|10
operator|+
name|ctx
operator|->
name|zstream
operator|.
name|total_out
operator|+
literal|8
expr_stmt|;
name|rc
operator|=
name|deflateEnd
argument_list|(
operator|&
name|ctx
operator|->
name|zstream
argument_list|)
expr_stmt|;
if|if
condition|(
name|rc
operator|!=
name|Z_OK
condition|)
block|{
name|ngx_log_error
argument_list|(
name|NGX_LOG_ALERT
argument_list|,
name|r
operator|->
name|connection
operator|->
name|log
argument_list|,
literal|0
argument_list|,
literal|"deflateEnd() failed: %d"
argument_list|,
name|rc
argument_list|)
expr_stmt|;
return|return
name|ngx_http_gzip_error
argument_list|(
name|ctx
argument_list|)
return|;
block|}
name|ngx_pfree
argument_list|(
name|r
operator|->
name|pool
argument_list|,
name|ctx
operator|->
name|preallocated
argument_list|)
expr_stmt|;
name|ctx
operator|->
name|flush
operator|=
name|Z_NO_FLUSH
expr_stmt|;
name|ngx_alloc_link_and_set_hunk
argument_list|(
name|cl
argument_list|,
name|ctx
operator|->
name|out_hunk
argument_list|,
name|r
operator|->
name|pool
argument_list|,
name|ngx_http_gzip_error
argument_list|(
name|ctx
argument_list|)
argument_list|)
expr_stmt|;
operator|*
name|ctx
operator|->
name|last_out
operator|=
name|cl
expr_stmt|;
name|ctx
operator|->
name|last_out
operator|=
operator|&
name|cl
operator|->
name|next
expr_stmt|;
if|if
condition|(
name|ctx
operator|->
name|zstream
operator|.
name|avail_out
operator|>=
literal|8
condition|)
block|{
name|trailer
operator|=
operator|(
expr|struct
name|gztrailer
operator|*
operator|)
name|ctx
operator|->
name|out_hunk
operator|->
name|last
expr_stmt|;
name|ctx
operator|->
name|out_hunk
operator|->
name|type
operator||=
name|NGX_HUNK_LAST
expr_stmt|;
name|ctx
operator|->
name|out_hunk
operator|->
name|last
operator|+=
literal|8
expr_stmt|;
block|}
else|else
block|{
name|ngx_test_null
argument_list|(
name|h
argument_list|,
name|ngx_create_temp_hunk
argument_list|(
name|r
operator|->
name|pool
argument_list|,
literal|8
argument_list|)
argument_list|,
name|ngx_http_gzip_error
argument_list|(
name|ctx
argument_list|)
argument_list|)
expr_stmt|;
name|h
operator|->
name|type
operator||=
name|NGX_HUNK_LAST
expr_stmt|;
name|ngx_alloc_link_and_set_hunk
argument_list|(
name|cl
argument_list|,
name|h
argument_list|,
name|r
operator|->
name|pool
argument_list|,
name|ngx_http_gzip_error
argument_list|(
name|ctx
argument_list|)
argument_list|)
expr_stmt|;
operator|*
name|ctx
operator|->
name|last_out
operator|=
name|cl
expr_stmt|;
name|ctx
operator|->
name|last_out
operator|=
operator|&
name|cl
operator|->
name|next
expr_stmt|;
name|trailer
operator|=
operator|(
expr|struct
name|gztrailer
operator|*
operator|)
name|h
operator|->
name|pos
expr_stmt|;
name|h
operator|->
name|last
operator|+=
literal|8
expr_stmt|;
block|}
if|#
directive|if
operator|(
name|HAVE_LITTLE_ENDIAN
operator|)
name|trailer
operator|->
name|crc32
operator|=
name|ctx
operator|->
name|crc32
expr_stmt|;
name|trailer
operator|->
name|zlen
operator|=
name|ctx
operator|->
name|zin
expr_stmt|;
else|#
directive|else
comment|/* STUB */
endif|#
directive|endif
name|ctx
operator|->
name|zstream
operator|.
name|avail_in
operator|=
literal|0
expr_stmt|;
name|ctx
operator|->
name|zstream
operator|.
name|avail_out
operator|=
literal|0
expr_stmt|;
name|ctx
operator|->
name|done
operator|=
literal|1
expr_stmt|;
if|#
directive|if
literal|0
block_content|ngx_http_delete_ctx(r, ngx_http_gzip_filter_module);
endif|#
directive|endif
break|break;
block|}
if|else if
condition|(
name|conf
operator|->
name|no_buffer
operator|&&
name|ctx
operator|->
name|in
operator|==
name|NULL
condition|)
block|{
name|ngx_alloc_link_and_set_hunk
argument_list|(
name|cl
argument_list|,
name|ctx
operator|->
name|out_hunk
argument_list|,
name|r
operator|->
name|pool
argument_list|,
name|ngx_http_gzip_error
argument_list|(
name|ctx
argument_list|)
argument_list|)
expr_stmt|;
operator|*
name|ctx
operator|->
name|last_out
operator|=
name|cl
expr_stmt|;
name|ctx
operator|->
name|last_out
operator|=
operator|&
name|cl
operator|->
name|next
expr_stmt|;
break|break;
block|}
block|}
block|}
if|if
condition|(
name|ctx
operator|->
name|out
operator|==
name|NULL
operator|&&
name|last
operator|!=
name|NGX_NONE
condition|)
block|{
return|return
name|last
return|;
block|}
name|last
operator|=
name|ngx_http_next_body_filter
argument_list|(
name|r
argument_list|,
name|ctx
operator|->
name|out
argument_list|)
expr_stmt|;
if|if
condition|(
name|last
operator|==
name|NGX_ERROR
condition|)
block|{
return|return
name|ngx_http_gzip_error
argument_list|(
name|ctx
argument_list|)
return|;
block|}
name|ngx_chain_update_chains
argument_list|(
operator|&
name|ctx
operator|->
name|free
argument_list|,
operator|&
name|ctx
operator|->
name|busy
argument_list|,
operator|&
name|ctx
operator|->
name|out
argument_list|,
operator|(
name|ngx_hunk_tag_t
operator|)
operator|&
name|ngx_http_gzip_filter_module
argument_list|)
expr_stmt|;
name|ctx
operator|->
name|last_out
operator|=
operator|&
name|ctx
operator|->
name|out
expr_stmt|;
block|}
block|}
end_function

begin_function
DECL|function|ngx_http_gzip_filter_alloc (void * opaque,u_int items,u_int size)
specifier|static
name|void
modifier|*
name|ngx_http_gzip_filter_alloc
parameter_list|(
name|void
modifier|*
name|opaque
parameter_list|,
name|u_int
name|items
parameter_list|,
name|u_int
name|size
parameter_list|)
block|{
name|ngx_http_gzip_ctx_t
modifier|*
name|ctx
init|=
name|opaque
decl_stmt|;
name|void
modifier|*
name|p
decl_stmt|;
name|ngx_uint_t
name|alloc
decl_stmt|;
name|alloc
operator|=
name|items
operator|*
name|size
expr_stmt|;
if|if
condition|(
name|alloc
operator|%
literal|512
operator|!=
literal|0
condition|)
block|{
comment|/* we allocate 8K for zlib deflate_state (~6K) */
comment|/* TODO: PAGE_SIZE */
name|alloc
operator|=
operator|(
name|alloc
operator|+
literal|4095
operator|)
operator|&
operator|~
literal|4095
expr_stmt|;
block|}
if|if
condition|(
name|alloc
operator|<=
name|ctx
operator|->
name|allocated
condition|)
block|{
name|p
operator|=
name|ctx
operator|->
name|free_mem
expr_stmt|;
name|ctx
operator|->
name|free_mem
operator|+=
name|alloc
expr_stmt|;
name|ctx
operator|->
name|allocated
operator|-=
name|alloc
expr_stmt|;
name|ngx_log_debug4
argument_list|(
name|NGX_LOG_DEBUG_HTTP
argument_list|,
name|ctx
operator|->
name|request
operator|->
name|connection
operator|->
name|log
argument_list|,
literal|0
argument_list|,
literal|"gzip alloc: n:%d s:%d a:%d p:"
name|PTR_FMT
argument_list|,
name|items
argument_list|,
name|size
argument_list|,
name|alloc
argument_list|,
name|p
argument_list|)
expr_stmt|;
return|return
name|p
return|;
block|}
name|ngx_log_error
argument_list|(
name|NGX_LOG_ALERT
argument_list|,
name|ctx
operator|->
name|request
operator|->
name|connection
operator|->
name|log
argument_list|,
literal|0
argument_list|,
literal|"gzip filter failed to use preallocated memory: %d of %d"
argument_list|,
name|items
operator|*
name|size
argument_list|,
name|ctx
operator|->
name|allocated
argument_list|)
expr_stmt|;
name|p
operator|=
name|ngx_palloc
argument_list|(
name|ctx
operator|->
name|request
operator|->
name|pool
argument_list|,
name|items
operator|*
name|size
argument_list|)
expr_stmt|;
return|return
name|p
return|;
block|}
end_function

begin_function
DECL|function|ngx_http_gzip_filter_free (void * opaque,void * address)
specifier|static
name|void
name|ngx_http_gzip_filter_free
parameter_list|(
name|void
modifier|*
name|opaque
parameter_list|,
name|void
modifier|*
name|address
parameter_list|)
block|{
if|#
directive|if
literal|0
block_content|ngx_http_gzip_ctx_t *ctx = opaque;      ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ctx->request->connection->log, 0,                    "gzip free: %X", address);
endif|#
directive|endif
block|}
end_function

begin_function
DECL|function|ngx_http_gzip_log_ratio (ngx_http_request_t * r,u_char * buf,uintptr_t data)
specifier|static
name|u_char
modifier|*
name|ngx_http_gzip_log_ratio
parameter_list|(
name|ngx_http_request_t
modifier|*
name|r
parameter_list|,
name|u_char
modifier|*
name|buf
parameter_list|,
name|uintptr_t
name|data
parameter_list|)
block|{
name|ngx_uint_t
name|zint
decl_stmt|,
name|zfrac
decl_stmt|;
name|ngx_http_gzip_ctx_t
modifier|*
name|ctx
decl_stmt|;
name|ctx
operator|=
name|ngx_http_get_module_ctx
argument_list|(
name|r
argument_list|,
name|ngx_http_gzip_filter_module
argument_list|)
expr_stmt|;
if|if
condition|(
name|ctx
operator|==
name|NULL
operator|||
name|ctx
operator|->
name|zout
operator|==
literal|0
condition|)
block|{
operator|*
name|buf
operator|=
literal|'-'
expr_stmt|;
return|return
name|buf
operator|+
literal|1
return|;
block|}
if|#
directive|if
literal|0
block_content|return buf + ngx_snprintf((char *) buf, NGX_INT32_LEN + 4, "%.2f",                               (float) ctx->zin / ctx->zout);
endif|#
directive|endif
comment|/* we prefer do not use FPU */
name|zint
operator|=
operator|(
name|ngx_uint_t
operator|)
operator|(
name|ctx
operator|->
name|zin
operator|/
name|ctx
operator|->
name|zout
operator|)
expr_stmt|;
name|zfrac
operator|=
operator|(
name|ngx_uint_t
operator|)
operator|(
operator|(
name|ctx
operator|->
name|zin
operator|*
literal|100
operator|/
name|ctx
operator|->
name|zout
operator|)
operator|%
literal|100
operator|)
expr_stmt|;
if|if
condition|(
operator|(
name|ctx
operator|->
name|zin
operator|*
literal|1000
operator|/
name|ctx
operator|->
name|zout
operator|)
operator|%
literal|10
operator|>
literal|4
condition|)
block|{
if|if
condition|(
operator|++
name|zfrac
operator|>
literal|99
condition|)
block|{
name|zint
operator|++
expr_stmt|;
name|zfrac
operator|=
literal|0
expr_stmt|;
block|}
block|}
return|return
name|buf
operator|+
name|ngx_snprintf
argument_list|(
operator|(
name|char
operator|*
operator|)
name|buf
argument_list|,
name|NGX_INT32_LEN
operator|+
literal|4
argument_list|,
literal|"%d.%02d"
argument_list|,
name|zint
argument_list|,
name|zfrac
argument_list|)
return|;
block|}
end_function

begin_function
DECL|function|ngx_http_gzip_error (ngx_http_gzip_ctx_t * ctx)
name|ngx_inline
specifier|static
name|int
name|ngx_http_gzip_error
parameter_list|(
name|ngx_http_gzip_ctx_t
modifier|*
name|ctx
parameter_list|)
block|{
name|deflateEnd
argument_list|(
operator|&
name|ctx
operator|->
name|zstream
argument_list|)
expr_stmt|;
name|ngx_pfree
argument_list|(
name|ctx
operator|->
name|request
operator|->
name|pool
argument_list|,
name|ctx
operator|->
name|preallocated
argument_list|)
expr_stmt|;
name|ctx
operator|->
name|zstream
operator|.
name|avail_in
operator|=
literal|0
expr_stmt|;
name|ctx
operator|->
name|zstream
operator|.
name|avail_out
operator|=
literal|0
expr_stmt|;
return|return
name|NGX_ERROR
return|;
block|}
end_function

begin_function
DECL|function|ngx_http_gzip_pre_conf (ngx_conf_t * cf)
specifier|static
name|int
name|ngx_http_gzip_pre_conf
parameter_list|(
name|ngx_conf_t
modifier|*
name|cf
parameter_list|)
block|{
name|ngx_http_log_op_name_t
modifier|*
name|op
decl_stmt|;
for|for
control|(
name|op
operator|=
name|ngx_http_gzip_log_fmt_ops
init|;
name|op
operator|->
name|name
operator|.
name|len
condition|;
name|op
operator|++
control|)
block|{
comment|/* void */
block|}
name|op
operator|->
name|op
operator|=
name|NULL
expr_stmt|;
name|op
operator|=
name|ngx_http_log_fmt_ops
expr_stmt|;
for|for
control|(
name|op
operator|=
name|ngx_http_log_fmt_ops
init|;
name|op
operator|->
name|op
condition|;
name|op
operator|++
control|)
block|{
if|if
condition|(
name|op
operator|->
name|name
operator|.
name|len
operator|==
literal|0
condition|)
block|{
name|op
operator|=
operator|(
name|ngx_http_log_op_name_t
operator|*
operator|)
name|op
operator|->
name|op
expr_stmt|;
block|}
block|}
name|op
operator|->
name|op
operator|=
operator|(
name|ngx_http_log_op_pt
operator|)
name|ngx_http_gzip_log_fmt_ops
expr_stmt|;
return|return
name|NGX_OK
return|;
block|}
end_function

begin_function
DECL|function|ngx_http_gzip_filter_init (ngx_cycle_t * cycle)
specifier|static
name|int
name|ngx_http_gzip_filter_init
parameter_list|(
name|ngx_cycle_t
modifier|*
name|cycle
parameter_list|)
block|{
name|ngx_http_next_header_filter
operator|=
name|ngx_http_top_header_filter
expr_stmt|;
name|ngx_http_top_header_filter
operator|=
name|ngx_http_gzip_header_filter
expr_stmt|;
name|ngx_http_next_body_filter
operator|=
name|ngx_http_top_body_filter
expr_stmt|;
name|ngx_http_top_body_filter
operator|=
name|ngx_http_gzip_body_filter
expr_stmt|;
return|return
name|NGX_OK
return|;
block|}
end_function

begin_function
DECL|function|ngx_http_gzip_create_conf (ngx_conf_t * cf)
specifier|static
name|void
modifier|*
name|ngx_http_gzip_create_conf
parameter_list|(
name|ngx_conf_t
modifier|*
name|cf
parameter_list|)
block|{
name|ngx_http_gzip_conf_t
modifier|*
name|conf
decl_stmt|;
name|ngx_test_null
argument_list|(
name|conf
argument_list|,
name|ngx_pcalloc
argument_list|(
name|cf
operator|->
name|pool
argument_list|,
sizeof|sizeof
argument_list|(
name|ngx_http_gzip_conf_t
argument_list|)
argument_list|)
argument_list|,
name|NGX_CONF_ERROR
argument_list|)
expr_stmt|;
name|conf
operator|->
name|enable
operator|=
name|NGX_CONF_UNSET
expr_stmt|;
name|conf
operator|->
name|no_buffer
operator|=
name|NGX_CONF_UNSET
expr_stmt|;
comment|/* conf->bufs.num = 0; */
name|conf
operator|->
name|http_version
operator|=
name|NGX_CONF_UNSET_UINT
expr_stmt|;
name|conf
operator|->
name|proxied
operator|=
name|NGX_CONF_UNSET_UINT
expr_stmt|;
name|conf
operator|->
name|level
operator|=
name|NGX_CONF_UNSET
expr_stmt|;
name|conf
operator|->
name|wbits
operator|=
name|NGX_CONF_UNSET
expr_stmt|;
name|conf
operator|->
name|memlevel
operator|=
name|NGX_CONF_UNSET
expr_stmt|;
return|return
name|conf
return|;
block|}
end_function

begin_function
DECL|function|ngx_http_gzip_merge_conf (ngx_conf_t * cf,void * parent,void * child)
specifier|static
name|char
modifier|*
name|ngx_http_gzip_merge_conf
parameter_list|(
name|ngx_conf_t
modifier|*
name|cf
parameter_list|,
name|void
modifier|*
name|parent
parameter_list|,
name|void
modifier|*
name|child
parameter_list|)
block|{
name|ngx_http_gzip_conf_t
modifier|*
name|prev
init|=
name|parent
decl_stmt|;
name|ngx_http_gzip_conf_t
modifier|*
name|conf
init|=
name|child
decl_stmt|;
name|ngx_conf_merge_value
argument_list|(
name|conf
operator|->
name|enable
argument_list|,
name|prev
operator|->
name|enable
argument_list|,
literal|0
argument_list|)
expr_stmt|;
name|ngx_conf_merge_bufs_value
argument_list|(
name|conf
operator|->
name|bufs
argument_list|,
name|prev
operator|->
name|bufs
argument_list|,
literal|4
argument_list|,
comment|/* STUB: PAGE_SIZE */
literal|4096
argument_list|)
expr_stmt|;
name|ngx_conf_merge_unsigned_value
argument_list|(
name|conf
operator|->
name|http_version
argument_list|,
name|prev
operator|->
name|http_version
argument_list|,
name|NGX_HTTP_VERSION_11
argument_list|)
expr_stmt|;
name|ngx_conf_merge_unsigned_value
argument_list|(
name|conf
operator|->
name|proxied
argument_list|,
name|prev
operator|->
name|proxied
argument_list|,
name|NGX_HTTP_GZIP_PROXIED_OFF
argument_list|)
expr_stmt|;
name|ngx_conf_merge_value
argument_list|(
name|conf
operator|->
name|level
argument_list|,
name|prev
operator|->
name|level
argument_list|,
literal|1
argument_list|)
expr_stmt|;
name|ngx_conf_merge_value
argument_list|(
name|conf
operator|->
name|wbits
argument_list|,
name|prev
operator|->
name|wbits
argument_list|,
name|MAX_WBITS
argument_list|)
expr_stmt|;
name|ngx_conf_merge_value
argument_list|(
name|conf
operator|->
name|memlevel
argument_list|,
name|prev
operator|->
name|memlevel
argument_list|,
name|MAX_MEM_LEVEL
operator|-
literal|1
argument_list|)
expr_stmt|;
name|ngx_conf_merge_value
argument_list|(
name|conf
operator|->
name|no_buffer
argument_list|,
name|prev
operator|->
name|no_buffer
argument_list|,
literal|0
argument_list|)
expr_stmt|;
return|return
name|NGX_CONF_OK
return|;
block|}
end_function

begin_function
DECL|function|ngx_http_gzip_set_window (ngx_conf_t * cf,void * post,void * data)
specifier|static
name|char
modifier|*
name|ngx_http_gzip_set_window
parameter_list|(
name|ngx_conf_t
modifier|*
name|cf
parameter_list|,
name|void
modifier|*
name|post
parameter_list|,
name|void
modifier|*
name|data
parameter_list|)
block|{
name|int
modifier|*
name|np
init|=
name|data
decl_stmt|;
name|int
name|wbits
decl_stmt|,
name|wsize
decl_stmt|;
name|ngx_conf_log_error
argument_list|(
name|NGX_LOG_INFO
argument_list|,
name|cf
argument_list|,
literal|0
argument_list|,
literal|"WBITS: %d"
argument_list|,
operator|*
name|np
argument_list|)
expr_stmt|;
name|wbits
operator|=
literal|15
expr_stmt|;
for|for
control|(
name|wsize
operator|=
literal|32
operator|*
literal|1024
init|;
name|wsize
operator|>
literal|256
condition|;
name|wsize
operator|>>=
literal|1
control|)
block|{
if|if
condition|(
name|wsize
operator|==
operator|*
name|np
condition|)
block|{
operator|*
name|np
operator|=
name|wbits
expr_stmt|;
name|ngx_conf_log_error
argument_list|(
name|NGX_LOG_INFO
argument_list|,
name|cf
argument_list|,
literal|0
argument_list|,
literal|"WBITS: %d"
argument_list|,
operator|*
name|np
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
name|wbits
operator|--
expr_stmt|;
block|}
return|return
literal|"must be 512, 1k, 2k, 4k, 8k, 16k, or 32k"
return|;
block|}
end_function

begin_function
DECL|function|ngx_http_gzip_set_hash (ngx_conf_t * cf,void * post,void * data)
specifier|static
name|char
modifier|*
name|ngx_http_gzip_set_hash
parameter_list|(
name|ngx_conf_t
modifier|*
name|cf
parameter_list|,
name|void
modifier|*
name|post
parameter_list|,
name|void
modifier|*
name|data
parameter_list|)
block|{
name|int
modifier|*
name|np
init|=
name|data
decl_stmt|;
name|int
name|memlevel
decl_stmt|,
name|hsize
decl_stmt|;
name|ngx_conf_log_error
argument_list|(
name|NGX_LOG_INFO
argument_list|,
name|cf
argument_list|,
literal|0
argument_list|,
literal|"MEMLEVEL: %d"
argument_list|,
operator|*
name|np
argument_list|)
expr_stmt|;
name|memlevel
operator|=
literal|9
expr_stmt|;
for|for
control|(
name|hsize
operator|=
literal|128
operator|*
literal|1024
init|;
name|hsize
operator|>
literal|256
condition|;
name|hsize
operator|>>=
literal|1
control|)
block|{
if|if
condition|(
name|hsize
operator|==
operator|*
name|np
condition|)
block|{
operator|*
name|np
operator|=
name|memlevel
expr_stmt|;
name|ngx_conf_log_error
argument_list|(
name|NGX_LOG_INFO
argument_list|,
name|cf
argument_list|,
literal|0
argument_list|,
literal|"MEMLEVEL: %d"
argument_list|,
operator|*
name|np
argument_list|)
expr_stmt|;
return|return
name|NULL
return|;
block|}
name|memlevel
operator|--
expr_stmt|;
block|}
return|return
literal|"must be 512, 1k, 2k, 4k, 8k, 16k, 32k, 64k, or 128k"
return|;
block|}
end_function

end_unit

