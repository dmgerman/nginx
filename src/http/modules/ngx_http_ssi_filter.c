...
begin_typedef
DECL|struct|__anon27cc9de50108
typedef|typedef
struct|struct
block|{
DECL|member|param
...
directive|if
literal|0
block_content|add in to ctx->out chain      while (ctx->out) {         rc = ngx_http_ssi_parse(r, ctx, ctx->out->hunk);          if (rc == NGX_HTTP_SSI_DONE) {             chain.hunk = ctx->out->hunk;             chain.next = NULL;              rc = next_body_filter(r,&chain);             if (rc != NGX_OK) {                 return rc;             }         }          ctx->out = ctx->out->next;     }
endif|#
directive|endif
return|return
name|NGX_OK
...
directive|if
literal|0
block_content|next_header_filter = ngx_http_top_header_filter;     ngx_http_top_header_filter = ngx_http_ssi_header_filter;
endif|#
directive|endif
name|next_body_filter
operator|=
