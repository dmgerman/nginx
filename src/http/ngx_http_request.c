...
specifier|static
name|void
name|ngx_http_wait_request_handler
parameter_list|(
name|ngx_event_t
modifier|*
name|ev
...
specifier|static
name|ngx_int_t
name|ngx_http_set_virtual_server
parameter_list|(
name|ngx_http_request_t
modifier|*
name|r
...
specifier|static
name|void
DECL|function|ngx_http_wait_request_handler (ngx_event_t * rev)
name|ngx_http_wait_request_handler
parameter_list|(
name|ngx_event_t
modifier|*
...
literal|0
argument_list|,
literal|"http wait request handler"
argument_list|)
expr_stmt|;
if|if
condition|(
...
return|return;
block|}
comment|/*          * We are trying to not hold c->buffer's memory for an idle connection.          */
if|if
condition|(
name|ngx_pfree
argument_list|(
...
name|ngx_http_request_t
modifier|*
DECL|function|ngx_http_create_request (ngx_connection_t * c)
name|ngx_http_create_request
parameter_list|(
name|ngx_connection_t
modifier|*
...
name|ngx_http_request_t
modifier|*
DECL|function|ngx_http_alloc_request (ngx_connection_t * c)
name|ngx_http_alloc_request
parameter_list|(
name|ngx_connection_t
modifier|*
...
begin_function
name|int
DECL|function|ngx_http_ssl_certificate (ngx_ssl_conn_t * ssl_conn,void * arg)
...
begin_function
name|ngx_int_t
DECL|function|ngx_http_process_request_uri (ngx_http_request_t * r)
name|ngx_http_process_request_uri
parameter_list|(
name|ngx_http_request_t
modifier|*
...
literal|0
argument_list|,
literal|"too large header to copy"
argument_list|)
expr_stmt|;
return|return
name|NGX_ERROR
...
literal|0
argument_list|,
literal|"client sent duplicate host header: \"%V: %V\", "
literal|"previous value: \"%V: %V\""
argument_list|,
operator|&
name|h
...
literal|0
argument_list|,
literal|"client sent HTTP/1.0 request with "
literal|"\"Transfer-Encoding\" header"
argument_list|)
expr_stmt|;
name|ngx_http_finalize_request
argument_list|(
...
literal|0
argument_list|,
literal|"client sent \"Content-Length\" and "
literal|"\"Transfer-Encoding\" headers "
...
literal|0
argument_list|,
literal|"client sent CONNECT method"
argument_list|)
expr_stmt|;
name|ngx_http_finalize_request
argument_list|(
...
literal|0
argument_list|,
literal|"client SSL certificate verify error: %s"
argument_list|,
name|s
argument_list|)
expr_stmt|;
...
specifier|static
name|ngx_int_t
DECL|function|ngx_http_validate_host (ngx_str_t * host,ngx_pool_t * pool,ngx_uint_t alloc)
name|ngx_http_validate_host
parameter_list|(
name|ngx_str_t
modifier|*
...
specifier|static
name|ngx_int_t
DECL|function|ngx_http_set_virtual_server (ngx_http_request_t * r,ngx_str_t * host)
name|ngx_http_set_virtual_server
parameter_list|(
name|ngx_http_request_t
modifier|*
...
name|r
argument_list|,
name|NGX_HTTP_MISDIRECTED_REQUEST
argument_list|)
expr_stmt|;
return|return
name|NGX_ERROR
...
specifier|static
name|ngx_int_t
DECL|function|ngx_http_find_virtual_server (ngx_connection_t * c,ngx_http_virtual_names_t * virtual_names,ngx_str_t * host,ngx_http_request_t * r,ngx_http_core_srv_conf_t ** cscfp)
name|ngx_http_find_virtual_server
parameter_list|(
name|ngx_connection_t
modifier|*
...
condition|)
block|{
comment|/*              * If the large header buffers were allocated while the previous              * request processing then we do not use c->buffer for              * the pipelined request (see ngx_http_create_request()).              *              * Now we would move the large header buffers to the free list.              */
for|for
control|(
name|cl
operator|=
...
block|}
block|}
comment|/* guard against recursive call from ngx_http_finalize_connection() */
name|r
operator|->
name|keepalive
operator|=
...
return|return;
block|}
comment|/*      * To keep a memory footprint as small as possible for an idle keepalive      * connection we try to free c->buffer's memory if it was allocated outside      * the c->pool.  The large header buffers are always allocated outside the      * c->pool and are freed too.      */
name|b
operator|=
name|c
operator|->
...
specifier|static
name|void
DECL|function|ngx_http_set_lingering_close (ngx_connection_t * c)
name|ngx_http_set_lingering_close
parameter_list|(
name|ngx_connection_t
modifier|*
...
literal|1
expr_stmt|;
comment|/*      * Setting r->pool to NULL will increase probability to catch double close      * of request since the request object is allocated from its own pool.      */
name|pool
operator|=
name|r
operator|->
