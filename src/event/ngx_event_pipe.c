...
break|break;
block|}
comment|/*          * bsize is the size of the busy hunks,          * to_write is the size of data in these hunks that          * would be written to a socket          */
name|bsize
operator|=
literal|0
expr_stmt|;
...
directive|if
literal|0
block_content|if (!(p->upstream_eof || p->upstream_error || p->upstream_done)) {
endif|#
directive|endif
for|for
control|(
...
directive|if
literal|0
block_content|if (!(p->upstream_eof || p->upstream_error || p->upstream_done)&& (bsize + ngx_hunk_size(cl->hunk)> p->busy_size))                 {                     break;                 }
else|#
directive|else
if|if
condition|(
...
literal|0
argument_list|,
literal|"pipe busy hunk data to write: %d"
argument_list|,
name|to_write
argument_list|)
expr_stmt|;
