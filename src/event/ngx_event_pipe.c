...
condition|)
block|{
comment|/*                  * if it's allowed then save some bufs from r->in                  * to a temporary file, and add them to a r->out chain                  */
name|rc
operator|=
name|ngx_event_pipe_write_chain_to_temp_file
argument_list|(
...
block|{
comment|/* if there're no bufs to read in then disable a level event */
name|ngx_log_debug0
argument_list|(
name|NGX_LOG_DEBUG_EVENT
argument_list|,
...
break|break;
block|}
comment|/* bsize is the size of the busy bufs */
name|bsize
operator|=
literal|0
expr_stmt|;
...
directive|if
literal|0
block_content|b->type&= ~(NGX_HUNK_TEMP|NGX_HUNK_IN_MEMORY|NGX_HUNK_RECYCLED);
endif|#
directive|endif
name|b
operator|->
...
directive|if
literal|0
block_content|b->type&= ~(NGX_HUNK_TEMP                  |NGX_HUNK_IN_MEMORY                  |NGX_HUNK_RECYCLED                  |NGX_HUNK_LAST_SHADOW);
endif|#
directive|endif
name|b
operator|->
