...
literal|0
expr_stmt|;
comment|/* The event addition or change should be always passed to a kernel        because there can be case when event was passed to a kernel then        added again to the change_list and then deleted from the change_list        by ngx_kqueue_del_event() so the first event still remains in a kernel */
if|#
directive|if
literal|0
block_content|if (nchanges> 0&& ev->index< nchanges&& change_list[ev->index].udata == ev)     {
if|#
directive|if
operator|(
name|NGX_DEBUG_EVENT
operator|)
block_content|ngx_connection_t *c = (ngx_connection_t *) ev->data;         ngx_log_debug(ev->log, "kqueue add event: %d: ft:%d" _ c->fd _ event);
endif|#
directive|endif
comment|/* if the event is still not passed to a kernel we change it */
block_content|change_list[ev->index].filter = event;         change_list[ev->index].flags = flags;          return NGX_OK;     }
endif|#
directive|endif
return|return
...
return|;
block|}
comment|/* when a socket is closed kqueue automatically deletes its filters         so we do not need to delete a event explicity before a socket closing */
if|if
condition|(
name|flags
operator|&
...
name|delta
expr_stmt|;
comment|/* Expired timers must be deleted before the events processing            because the new timers can be added during the processing */
...
name|udata
expr_stmt|;
comment|/* It's a stale event from a socket            that was just closed in this iteration */
if|if
condition|(
operator|!
name|ev
...
condition|)
block|{
comment|/* It's a stale event from a socket that was just closed                        in this iteration and during processing another socket                        was opened with the same number by accept() or socket()                        and its event has been added the event to the change_list                        but has not been passed to a kernel.  Nevertheless                        there's small chance that ngx_kqueue_set_event() has                        flushed the new event if the change_list was filled up.                        In this very rare case we would get EAGAIN while                        a reading or a writing */
continue|continue;
block|}
else|else
block|{
