...
begin_comment
comment|/*  * TODO: eliminate mutex and use atomic_xchg():  *       ev->next = ev; ngx_atomic_xchg(ngx_posted_events, ev->next);  *       in ngx_event_busy_unlock() and ngx_event_busy_lock_handler()  */
end_comment
...
expr_stmt|;
block|}
comment|/*      * MP:      * nocachable (bl->md5 == NULL): ngx_shared_mutex_unlock(mutex, !wakeup)      * cachable (bl->md5): ???      */
if|if
condition|(
name|wakeup
operator|==
