...
begin_typedef
DECL|struct|__anon28e53aa60108
typedef|typedef
struct|struct
block|{
DECL|member|engine
...
comment|/*          * SSL_CTX_add0_chain_cert() is needed to add chain to          * a particular certificate when multiple certificates are used;          * only available in OpenSSL 1.0.2+          */
if|if
condition|(
...
literal|0
argument_list|,
literal|"SSL_CTX_add0_chain_cert(\"%s\") failed"
argument_list|,
name|cert
operator|->
name|data
...
literal|0
argument_list|,
literal|"ENGINE_load_private_key(\"%s\") failed"
argument_list|,
name|last
argument_list|)
expr_stmt|;
...
literal|0
argument_list|,
literal|"loading \"engine:...\" certificate keys "
